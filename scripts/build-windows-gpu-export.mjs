...
const bundledExePath = path.join(bundledDir, "quiro-gpu-export.exe");
const helperId = "quiro-gpu-export";
const generatorArch = process.arch === "arm64" ? "ARM64" : "x64";

if (process.platform !== "win32") {
  console.log(
    "[build-windows-gpu-export] Skipping Windows GPU export helper build.",
  );
  process.exit(0);
}
...
if (!existsSync(path.join(sourceDir, "CMakeLists.txt"))) {
  console.error(
    "[build-windows-gpu-export] CMakeLists.txt not found at",
    sourceDir,
  );
  process.exit(1);
}
...
if (!cmake) {
  if (existsSync(bundledExePath)) {
    const verification = verifyNativeHelperManifest({
      ...
      binaryName: "quiro-gpu-export.exe",
    });
    if (!verification.ok) {
      console.error(
        formatNativeHelperManifestWarning(
          "build-windows-gpu-export",
          verification,
        ),
      );
      process.exit(1);
    ...
    console.log(
      `[build-windows-gpu-export] Using bundled helper: ${bundledExePath}`,
    );
    process.exit(0);
  }
...
try {
  ...
  console.log(
    "[build-windows-gpu-export] VS 2022 generator not found, trying VS 2019...",
  );
  try {
    clearCmakeCache();
    ...
    console.error(
      "[build-windows-gpu-export] CMake configure failed:",
      error.message,
    );
    process.exit(1);
  }
...
if (!existsSync(exePath)) {
  console.error(
    "[build-windows-gpu-export] Expected exe not found at",
    exePath,
  );
  process.exit(1);
}
...
console.log(
  `[build-windows-gpu-export] Staged bundled helper: ${bundledExePath}`,
);
...
console.log(
  `[build-windows-gpu-export] Updated helper manifest: ${manifestPath}`,
);
