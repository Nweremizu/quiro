...
const helpers = [
  {
    ...
    output: "quiro-screencapturekit-helper",
  },
  {
    source: "ScreenCaptureKitWindowList.swift",
    output: "quiro-window-list",
  },
  {
    source: "SystemCursorAssets.swift",
    output: "quiro-system-cursors",
  },
  {
    source: "NativeCursorMonitor.swift",
    output: "quiro-native-cursor-monitor",
  },
];
  ...
  for (const helper of helpers) {
    ...
    if (result.status !== 0) {
      const details = [result.stderr, result.stdout]
        ...
        .trim();
      throw new Error(
        details || `Failed to compile ${helper.source} for ${target.archTag}`,
      );
    }
