...
const helperId = "quiro-nvidia-cuda-compositor";
const generatorArch = process.arch === "arm64" ? "ARM64" : "x64";
const videoCodecSdkRoot =
  process.env.QUIRO_NVIDIA_VIDEO_CODEC_SDK_ROOT?.trim() ||
  path.join(projectRoot, ".tmp", "video-sdk-samples");

if (process.platform !== "win32") {
  console.log(
    "[build-nvidia-cuda-compositor] Skipping NVIDIA CUDA compositor build.",
  );
  process.exit(0);
}
...
if (!existsSync(path.join(sourceDir, "CMakeLists.txt"))) {
  console.error(
    "[build-nvidia-cuda-compositor] CMakeLists.txt not found at",
    sourceDir,
  );
  process.exit(1);
}
...
function fallbackToBundledHelperOrExit(reason) {
  if (existsSync(bundledExePath)) {
    const verification = verifyNativeHelperManifest({
      ...
      binaryName: "quiro-nvidia-cuda-compositor.exe",
    });
    if (!verification.ok) {
      console.warn(
        formatNativeHelperManifestWarning(
          "build-nvidia-cuda-compositor",
          verification,
        ),
      );
    }
    ...
    console.log(
      `[build-nvidia-cuda-compositor] Using bundled helper: ${bundledExePath}`,
    );
    process.exit(0);
  }
...
if (!existsSync(path.join(videoCodecSdkRoot, "Samples", "NvCodec"))) {
  fallbackToBundledHelperOrExit(
    `NVIDIA Video Codec SDK samples not found at ${videoCodecSdkRoot}. Set QUIRO_NVIDIA_VIDEO_CODEC_SDK_ROOT to build from source.`,
  );
}
...
try {
  clearCmakeCache();
  execSync(
    `${cmake} .. -G "Visual Studio 17 2022" -A ${generatorArch} -DQUIRO_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
    {
      cwd: buildDir,
      stdio: "inherit",
      timeout: 120000,
  ...
  console.log(
    "[build-nvidia-cuda-compositor] VS 2022 generator not found, trying VS 2019...",
  );
  try {
    clearCmakeCache();
    execSync(
      `${cmake} .. -G "Visual Studio 16 2019" -A ${generatorArch} -DQUIRO_NVIDIA_VIDEO_CODEC_SDK_ROOT="${videoCodecSdkRoot}"`,
      {
        cwd: buildDir,
        stdio: "inherit",
        timeout: 120000,
...
console.log(
  "[build-nvidia-cuda-compositor] Building NVIDIA CUDA compositor...",
);
try {
...
if (!existsSync(exePath)) {
  console.error(
    "[build-nvidia-cuda-compositor] Expected exe not found at",
    exePath,
  );
  process.exit(1);
}
...
console.log(
  `[build-nvidia-cuda-compositor] Staged bundled helper: ${bundledExePath}`,
);
...
console.log(
  `[build-nvidia-cuda-compositor] Updated helper manifest: ${manifestPath}`,
);
