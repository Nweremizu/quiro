 ...
 * extensions directory (~/.quiro/extensions/ or userData/extensions/).
 *
...
const MANIFEST_FILE_NAME = 'quiro-extension.json';
...
type PersistedExtensionStatus = Extract<
  ExtensionStatus,
...
function isPersistedExtensionStatus(
  value: unknown
): value is PersistedExtensionStatus {
  return value === 'active' || value === 'disabled' || value === 'installed';
}

async function readPersistedExtensionStatuses(): Promise<
  Record<string, PersistedExtensionStatus>
> {
  ...
  try {
    ...
    const entries = Object.entries(parsed).filter(
      ([key, value]) =>
        typeof key === 'string' && isPersistedExtensionStatus(value)
    );

    return Object.fromEntries(entries) as Record<
      string,
      PersistedExtensionStatus
    >;
  } catch {
    return {};
  }
...
function validateManifest(
  ...
  if (relativeMain.startsWith('..') || path.isAbsolute(relativeMain)) {
    console.warn(
      `[extensions] Extension ${m.id}: main entry escapes extension directory`
    );
    return null;
  }
  ...
  if (typeof m.author === 'string') result.author = m.author;
  if (typeof m.homepage === 'string') result.homepage = m.homepage;
  if (typeof m.license === 'string') result.license = m.license;
  if (typeof m.engine === 'string') result.engine = m.engine;
  if (typeof m.icon === 'string') result.icon = m.icon;
  if (typeof m.contributes === 'object' && m.contributes !== null) {
    result.contributes = m.contributes as NonNullable<
 ...
 */
async function scanExtensionsIn(
  directory: string,
  builtin: boolean
...
export async function discoverExtensions(): Promise<ExtensionInfo[]> {
  ...
  const applyPersistedStatus = (ext: ExtensionInfo): ExtensionInfo => {
    ...
    return {
      ...ext,
      status:
        persistedStatuses[ext.manifest.id] ??
        (ext.builtin ? 'active' : 'installed'),
    };
  };
