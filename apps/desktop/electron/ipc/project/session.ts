...
import { RECORDING_SESSION_MANIFEST_SUFFIX } from "@electron/utils/constants";
import type { RecordingSessionData, RecordingSessionManifest } from "@/types";
import { normalizeVideoSourcePath } from "@electron/utils";
...
export function getRecordingSessionManifestPath(videoPath: string) {
  ...
  return path.join(
    ...
    `${baseName}${RECORDING_SESSION_MANIFEST_SUFFIX}`,
  );
}
...
export async function persistRecordingSessionManifest(
  ...
  const normalizedWebcamPath = normalizeVideoSourcePath(
    session.webcamPath ?? null,
  );
...
export async function resolveLinkedWebcamPath(
  videoPath?: string | null,
): Promise<string | null> {
  const normalizedVideoPath = normalizeVideoSourcePath(videoPath);
  if (!normalizedVideoPath) {
  ...
  const candidateExtensions = Array.from(
    new Set(
      [extension, ".webm", ".mp4", ".mov", ".mkv", ".avi"].filter(Boolean),
    ),
  );
