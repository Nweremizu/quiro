...
import { isAutoRecordingPath, moveFileWithOverwrite } from "@electron/utils";
import {
...
export function waitForNativeCaptureStart(
  ...
  return new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup();
      reject(
        new Error("Timed out waiting for ScreenCaptureKit recorder to start"),
      );
    }, 12000);
...
export async function muxNativeMacRecordingWithAudio(
  ...
  for (const [label, audioPath] of [
    ...
    try {
      ...
      if (stat.size <= 0) {
        console.warn(
          `[mux] Skipping ${label} audio: file is empty (${audioPath})`,
        );
      ...
      console.warn(
        `[mux] Skipping ${label} audio: file not accessible (${audioPath})`,
      );
    }
  }
  ...
  if (videoDuration > 0) {
    for (let i = 0; i < audioFilePaths.length; i++) {
      const audioDuration = await probeMediaDurationSeconds(
        audioFilePaths[i] as string,
      );
      const adjustment = getAudioSyncAdjustment(videoDuration, audioDuration);
      audioAdjustments.set(availableAudioInputs[i] as string, adjustment);
  ...
  if (availableAudioInputs.length === 2) {
    ...
    const singleAdjustment = audioAdjustments.get(
      availableAudioInputs[0] as string,
    ) ?? {
      mode: "none",
      delayMs: 0,
      tempoRatio: 1,
...
export async function finalizeStoredVideo(videoPath: string) {
  ...
  if (videoPath.endsWith(".mp4")) {
    const companionCandidates =
      await getUsableCompanionAudioCandidates(videoPath);
  ...
  try {
    ...
    if (
      ...
      recordNativeCaptureDiagnostics({
        ...
        processOutput: lastNativeCaptureDiagnostics.processOutput as string,
...
export async function recoverNativeMacCaptureOutput() {
  ...
  const systemAudioPath =
    nativeCaptureSystemAudioPath ?? macDiagnostics?.systemAudioPath ?? null;
  const microphonePath =
    nativeCaptureMicrophonePath ?? macDiagnostics?.microphonePath ?? null;
  ...
  try {
    ...
    recordNativeCaptureDiagnostics({
      ...
      systemAudioPath,
      microphonePath,
      processOutput: (nativeCaptureOutputBuffer.trim() || undefined) as string,
