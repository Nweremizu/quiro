...
import { moveFileWithOverwrite } from "@electron/utils";
import {
...
export function waitForWindowsCaptureStart(
  ...
  return new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      cleanup();
      reject(
        new Error("Timed out waiting for native Windows capture to start"),
      );
    }, 12000);
...
export async function muxNativeWindowsVideoWithAudio(
  ...
  for (const [label, audioPath] of [
    ...
    try {
      ...
      if (stat.size <= 0) {
        console.warn(
          `[mux-win] Skipping ${label} audio: file is empty (${audioPath})`,
        );
      ...
      console.warn(
        `[mux-win] Skipping ${label} audio: file not accessible (${audioPath})`,
      );
    }
  }
  ...
  if (videoDuration > 0) {
    for (let i = 0; i < audioFilePaths.length; i++) {
      ...
      const recordedStartDelayMs = await getCompanionAudioStartDelayMs(
        audioFilePaths[i] as string,
      );
      ...
      audioAdjustments.set(audioInputs[i] as string, adjustment);
      if (
  ...
  try {
    if (audioInputs.length === 2) {
      ...
      filterParts.push(
        "[s][m]amix=inputs=2:duration=longest:normalize=0[aout]",
      );
