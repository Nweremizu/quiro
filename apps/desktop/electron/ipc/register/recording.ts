...
import { ALLOW_QUIRO_WINDOW_CAPTURE } from "@electron/utils/constants";
import {
...
import { rememberApprovedLocalReadPath } from "@electron/ipc/project/manager";
import {
...
import { shouldUseWindowsBrowserMicrophoneFallback } from "@electron/ipc/recording/windows-fallback";
import {
...
import { resolveWindowsCaptureDisplay } from "@electron/ipc/capture-selection";
  ...
  ipcMain.handle(
    ...
    async (_, source: SelectedSource, options?: NativeMacRecordingOptions) => {
      ...
      if (process.platform === "win32") {
        ...
        try {
          ...
          recordNativeCaptureDiagnostics({
            ...
            systemAudioPath,
            microphonePath,
            processOutput: (captureOutput.trim() || undefined) as string,
          });
          ...
          recordNativeCaptureDiagnostics({
            ...
            processOutput: (windowsCaptureOutputBuffer.trim() ||
              undefined) as string,
            error: String(error),
          });
      ...
      try {
        ...
        if (
          !ALLOW_QUIRO_WINDOW_CAPTURE &&
          source?.id?.startsWith("window:") &&
          appName &&
          (appName === ownAppName || appName === "quiro")
        ) {
          return {
            success: false,
            message:
              "Cannot record Quiro windows. Please select another app window.",
          };
        }
        ...
        recordNativeCaptureDiagnostics({
          ...
          processOutput: (nativeCaptureOutputBuffer.trim() ||
            undefined) as string,
        });
        return {
          success: true,
        ...
        if (
          ...
          const { response } = await dialog.showMessageBox({
            ...
            message:
              "Quiro needs screen recording permission to capture your screen.",
            detail:
              "Please open System Settings > Privacy & Security > Screen Recording, make sure Quiro is toggled ON, then try recording again.",
        ...
        if (errorStr.includes("MICROPHONE_PERMISSION_DENIED")) {
          const { response } = await dialog.showMessageBox({
            ...
            message: "Quiro needs microphone permission to record audio.",
            detail:
              "Please open System Settings > Privacy & Security > Microphone, make sure Quiro is toggled ON, then try recording again.",
            buttons: ["Open System Settings", "Cancel"],
            defaultId: 0,
            cancelId: 1,
          });
  ...
  ipcMain.handle("stop-native-screen-recording", async () => {
    ...
    try {
      ...
      recordNativeCaptureDiagnostics({
        ...
        osRelease: lastNativeCaptureDiagnostics?.osRelease as string,
        supported: lastNativeCaptureDiagnostics?.supported as boolean,
        helperExists: lastNativeCaptureDiagnostics?.helperExists as boolean,
        processOutput: (nativeCaptureOutputBuffer.trim() ||
          undefined) as string,
