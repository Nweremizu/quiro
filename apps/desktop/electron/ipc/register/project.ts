...
} from "@electron/utils/mediaServer";
import {
...
function normalizeProjectSaveName(projectName?: string | null) {
  ...
  const withoutInvalidFilesystemChars = withoutExtension.replace(
    /[<>:"/\\|?*]/g,
    "",
  );
...
async function ensureNamedProjectSaveDoesNotOverwriteDifferentProject(
  ...
  try {
    ...
    if (
      existingVideoPath &&
      incomingVideoPath &&
      existingVideoPath !== incomingVideoPath
    ) {
      return {
        success: false,
        message: "A different project already uses this name",
    ...
    console.error(
      "Failed to verify existing named project before overwrite:",
      error,
    );
    return {
      success: false,
...
export function registerProjectHandlers() {
  ipcMain.handle("reveal-in-folder", async (_, filePath: string) => {
    try {
      ...
      try {
        ...
        console.error(
          `Error opening directory: ${path.dirname(filePath)}`,
          openError,
        );
  ...
  ipcMain.handle("choose-recordings-directory", async () => {
    try {
      ...
      const selectedPath = path.resolve(result.filePaths[0] as string);
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const trustedExistingProjectPath = isTrustedProjectPath(
          existingProjectPath,
        )
          ? existingProjectPath
          : null;
        ...
        const safeName =
          normalizeProjectSaveName(suggestedName) || `project-${Date.now()}`;
        const defaultName = `${safeName}.${PROJECT_FILE_EXTENSION}`;

        const result = await dialog.showSaveDialog({
          title: "Save Quiro Project",
          defaultPath: path.join(projectsDir, defaultName),
          filters: [
            { name: "Quiro Project", extensions: [PROJECT_FILE_EXTENSION] },
  ...
  ipcMain.handle("load-project-file", async () => {
    try {
      ...
      return await loadProjectFromPath(result.filePaths[0] as string);
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const resolvedSession = (await resolveRecordingSession(
        currentVideoPath,
      )) ?? {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      setCurrentRecordingSession({
        ...
        hideOverlayCursorByDefault: normalizeBoolean(
          session.hideOverlayCursorByDefault,
        ),
      });
      await replaceApprovedSessionLocalReadPaths([
  ...
  ipcMain.handle("get-current-video-path", () => {
    return currentVideoPath
      ? { success: true, path: currentVideoPath }
      : { success: false };
  });
  ...
  ipcMain.handle("delete-recording-file", async (_, filePath: string) => {
    try {
      if (!filePath) {
        return {
          success: false,
          error: "Only auto-generated recordings can be deleted",
        };
      }
  ...
  ipcMain.handle("get-local-media-url", async (_, filePath: string) => {
    ...
    if (!resolved) {
      ...
      console.warn(
        `[get-local-media-url] Blocked disallowed path: ${normalized}`,
      );
      return { success: false as const };
    }
