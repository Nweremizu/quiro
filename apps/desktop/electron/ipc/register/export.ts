...
import {
  finalizeExportStreamSession,
  isOwnedExportPath,
  openExportStreamSession,
  registerOwnedExportPath,
  releaseOwnedExportPath,
  writeToExportStreamSession,
} from "@electron/ipc/export/export-stream";
import {
...
import { approveUserPath } from "@electron/utils";
...
export function registerExportHandlers() {
    ...
    async (
      ...
      try {
        ...
        if (inputMode === "h264-stream") {
          ...
          ffmpegArgs = buildNativeVideoExportArgs(
            encoderName,
            options,
            outputPath,
          );
  ...
  ipcMain.handle(
    ...
    async (_, sessionId: string, options?: NativeVideoExportFinishOptions) => {
      ...
      try {
        ...
        const finalizedSuffix = session.outputPath.replace(
          /\.mp4$/,
          "-final.mp4",
        );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        const result = await muxExportedVideoAudioBuffer(
          videoData,
          options ?? {},
        );
  ...
  ipcMain.handle(
    "export-stream-open",
    async (_event, options?: { extension?: string }) => {
      try {
        const result = await openExportStreamSession(options);
        return {
          success: true,
          streamId: result.sessionId,
  ...
  ipcMain.handle(
    ...
    async (_event, streamId: string, position: number, chunk: Uint8Array) => {
      try {
        await writeToExportStreamSession(streamId, position, chunk);
        return { success: true };
      } catch (error) {
        return { success: false, error: String(error) };
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (typeof tempPath !== "string" || typeof fileName !== "string") {
        return {
          success: false,
          error: "Invalid finalize-exported-video payload",
        };
      }
