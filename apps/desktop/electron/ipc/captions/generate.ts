...
export async function ensureReadableFile(
  ...
  if (options?.executable) {
    try {
      ...
      throw new Error(
        "The selected Whisper executable is not marked as executable.",
      );
    }
  }
}
...
export async function resolveWhisperExecutablePath(
  ...
  throw new Error(
    "No Whisper runtime was found. Quiro looked for a bundled binary first, then checked common system install locations.",
  );
}
...
export async function generateAutoCaptionsFromVideo(options: {
  ...
  const tempBase = path.join(
    app.getPath("temp"),
    `quiro-captions-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  );
