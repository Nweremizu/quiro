...
export type NativeVideoExportAudioMode =
  | 'none'
  | 'copy-source'
  | 'trim-source'
  | 'edited-track';
...
export function getNativeVideoInputByteSize(
  width: number,
  height: number
): number {
...
function getBitrateArgs(bitrate: number): string[] {
  ...
  const maxRate = Math.max(
    effectiveBitrate,
    Math.round(effectiveBitrate * 1.2)
  );
...
export function buildTrimmedSourceAudioFilter(
  ...
  if (segmentLabels.length === 1) {
    ...
    filterParts.push(
      `${segmentLabels.join('')}concat=n=${segmentLabels.length}:v=0:a=1[aout]`
    );
  }
