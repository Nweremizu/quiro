...
/**
 * Builds an array of FFmpeg `atempo` filters to achieve the target tempo ratio.
 * Since the `atempo` filter restricts ratio limits between 0.5 and 2.0,
 * this safely chains multiple filters for values outside that range.
 */
...
/**
 * Calculates audio synchronization adjustments to align audio tracks with video tracks.
 * Handles slight duration discrepancies using 'tempo' shifts, 'delay' offsets,
 * or 'pad' insertion depending on the magnitude of the delta.
 */
...
/**
 * Overrides or adjusts the initial computed AudioSyncAdjustment to account
 * for a known recorded audio start delay. If the recorded start delay is greater
 * than 20ms, it is applied directly. Otherwise, it converts adjustments to use
 * padding mode instead.
 */
...
/**
 * Appends synced audio filters (delay, tempo, pad, volume) to an array of FFmpeg filter
 * parts, routing processing from an `inputLabel` stream to an `outputLabel` stream.
 * Includes async resampling to guarantee proper timestamps.
 */
export function appendSyncedAudioFilter(
  ...
  const preFilters =
    typeof options === 'number' ? [] : (options.preFilters ?? []);
  ...
  if (adjustment.mode === 'pad' && adjustment.durationDeltaMs > 0) {
    filters.push(
      `apad=pad_dur=${formatFfmpegSeconds(adjustment.durationDeltaMs)}`
    );
  }
...
/**
 * Formats a given number of milliseconds as a decimal seconds string suitable for FFmpeg configuration.
 */
...
/**
 * Normalizes an array of `PauseSegment`s by filtering invalid times, enforcing logical bounds,
 * sorting them chronologically, and merging any overlapping segments.
 */
export function normalizePauseSegments(
  ...
  const normalized = pauseSegments
    .map((segment) => {
      ...
      return { startMs: clampedStart, endMs: clampedEnd } as PauseSegment;
    })
  ...
  const merged: PauseSegment[] = [{ ...normalized[0] } as PauseSegment];
  ...
  for (const segment of normalized.slice(1)) {
    ...
    if (segment.startMs <= previous.endMs) {
      ...
      merged.push({ ...segment } as PauseSegment);
    }
  }
...
/**
 * Generates an FFmpeg complex filter string to cut out paused portions of an audio track.
 * This function effectively splits the stream to keep the "active" segments, then
 * concatenates them to seamlessly bypass the `pauseSegments`.
 */
export function buildPausedAudioFilter(
  ...
  activeSegments.forEach((segment, index) => {
    ...
    filterParts.push(
      `[${inputLabel}]atrim=${trimArgs},asetpts=PTS-STARTPTS[${segmentLabel}]`
    );
...
/**
 * Parses the final output duration in seconds from FFmpeg stderr logging output.
 * Looks for lines matching: Duration: HH:MM:SS.ms
 */
