...
import {
  cachedNativeVideoEncoder,
  setCachedNativeVideoEncoder,
} from "@electron/ipc/state";
...
export const nativeVideoExportSessions = new Map<
  string,
  NativeVideoExportSession
>();
...
export async function removeTemporaryExportFile(
...
export function isIgnorableNativeVideoExportStreamError(
...
export async function waitForNativeVideoExportDrain(
  session: NativeVideoExportSession,
) {
  if (
    session.stdinError ||
  ...
  await new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          "Timed out while waiting for native export writer backpressure to clear",
        ),
      );
    }, 15000);
...
export function getNativeVideoExportFrameLength(
...
export async function writeNativeVideoExportFrame(
  ...
  if (
    session.ffmpegProcess.stdin.writableLength >= session.maxQueuedWriteBytes
  ) {
    try {
...
export async function probeNativeVideoEncoder(
  ...
  const outputPath = path.join(
    app.getPath("temp"),
    `quiro-export-probe-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
  );
...
export async function resolveNativeVideoEncoder(
  ...
  const candidates = [
    ...new Set([
      ...getPreferredNativeVideoEncoders(process.platform),
      "libx264",
    ]),
  ];
...
export async function muxNativeVideoExportAudio(
  ...
  const useEditedTrackFiltergraph =
    ...
    options.editedTrackStrategy === "filtergraph-fast-path";
  ...
  if (audioMode === "edited-track" && !useEditedTrackFiltergraph) {
    ...
    audioInputPath = path.join(
      app.getPath("temp"),
      `quiro-export-audio-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${extension}`,
    );
  ...
  if (audioMode === "trim-source") {
    ...
    if (!filter) {
      throw new Error(
        "Edited-track filtergraph inputs are incomplete for native export",
      );
    }
    args.push("-filter_complex", filter, "-map", "0:v:0", "-map", "[aout]");
  } else {
  ...
  try {
    ...
    await Promise.allSettled(
      tempArtifacts.map((artifactPath) =>
        removeTemporaryExportFile(artifactPath),
      ),
    );
  }
}
...
export async function muxExportedVideoAudioBuffer(
  ...
  const tempVideoPath = path.join(
    app.getPath("temp"),
    `quiro-export-video-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.mp4`,
  );
  const metrics: NativeVideoAudioMuxMetrics = {};
  let succeeded = false;
