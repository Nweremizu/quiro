...
import { getTelemetryPathForVideo, getScreen } from "@electron/utils";
import {
...
export function resumeCursorCapture(resumedAtMs: number) {
  ...
  const pauseDurationMs = Math.max(
    0,
    resumedAtMs - cursorCapturePauseStartedAtMs,
  );
  setCursorCaptureAccumulatedPausedMs(
...
export function getCursorCaptureElapsedMs(nowMs = Date.now()) {
  if (
    !Number.isFinite(cursorCaptureStartTimeMs) ||
    cursorCaptureStartTimeMs <= 0
  ) {
    return 0;
  }
...
export function getNormalizedCursorPoint() {
  ...
  const linuxCursorCache =
    process.platform === "linux" ? linuxCursorScreenPoint : null;
  const isLinuxCacheFresh =
    !!linuxCursorCache && Date.now() - linuxCursorCache.updatedAt <= 1000;
  ...
  const sourceDisplay = Number.isFinite(sourceDisplayId)
    ? (getScreen()
        .getAllDisplays()
        .find((display: { id: number }) => display.id === sourceDisplayId) ??
      null)
    : null;
...
export function getHookCursorScreenPoint(
  ...
  const rawX =
    event?.x ?? event?.data?.x ?? event?.screenX ?? event?.data?.screenX;
  const rawY =
    event?.y ?? event?.data?.y ?? event?.screenY ?? event?.data?.screenY;
...
export function snapshotCursorTelemetryForPersistence() {
  ...
  setPendingCursorSamples([
    ...pendingCursorSamples,
    ...activeCursorSamples.filter(
      (sample) => sample.timeMs > lastPendingTimeMs,
    ),
  ]);
}
...
export { getTelemetryPathForVideo } from "@electron/utils";
export { CURSOR_SAMPLE_INTERVAL_MS } from "@electron/utils/constants";
