...
function ensureNativeVideoExportWriteResultListener() {
  ...
  ipcRenderer.on(
    ...
    (
      ...
      const pendingRequest = nativeVideoExportWriteRequests.get(
        payload.requestId,
      );
      if (!pendingRequest) {
        return;
...
function settleNativeVideoExportPendingRequests(
  ...
  for (const [
    requestId,
    pendingRequest,
  ] of nativeVideoExportWriteRequests.entries()) {
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  nativeVideoExportCancel: (sessionId: string) => {
    return ipcRenderer
      .invoke("native-video-export-cancel", sessionId)
      .finally(() => {
  ...
  onSelectedSourceChanged: (
    ...
    return () =>
      ipcRenderer.removeListener("selected-source-changed", listener);
  },
  ...
  onStopRecordingFromTray: (callback: () => void) => {
    ...
    return () =>
      ipcRenderer.removeListener("stop-recording-from-tray", listener);
  },
  onRecordingStateChanged: (
    ...
    return () =>
      ipcRenderer.removeListener("recording-state-changed", listener);
  },
  ...
  onRequestSaveBeforeClose: (callback: () => Promise<boolean>) => {
    ...
    return () =>
      ipcRenderer.removeListener("request-save-before-close", listener);
  },
  ...
  getRecordingPreferences: () =>
    ipcRenderer.invoke("get-recording-preferences"),
  ...
  setCountdownDelay: (delay: number) =>
    ipcRenderer.invoke("set-countdown-delay", delay),
  startCountdown: (seconds: number) =>
    ipcRenderer.invoke("start-countdown", seconds),
  ...
  onCountdownTick: (callback: (seconds: number) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, seconds: number) =>
      callback(seconds);
  ...
  extensionsDisable: (id: string) =>
    ipcRenderer.invoke("extensions:disable", id),
  extensionsInstallFromFolder: () =>
  ...
  extensionsOpenDirectory: () =>
    ipcRenderer.invoke("extensions:open-directory"),
  ...
  extensionsMarketplaceGet: (id: string) =>
    ipcRenderer.invoke("extensions:marketplace-get", id),
