...
use base64::prelude::*;
use quiro_recording::screen_capture::ScreenCaptureTarget;
...
#[derive(tauri_specta::Event, Serialize, Type, Clone, PartialEq)]
pub struct TargetUnderCursor {
    display_id: Option<DisplayId>,
    window: Option<WindowUnderCursor>,
}

/// The most recent cursor target, mirrored here so a newly-opened overlay can
/// ask for it directly.
///
/// The poll loop only emits on *change* (see `open_target_select_overlays`), and
/// an overlay webview finishes loading well after that loop starts. Without this,
/// an overlay that opens under a stationary cursor never receives an event and
/// sits inert until the cursor moves to another display and back.
static LAST_TARGET_UNDER_CURSOR: Mutex<Option<TargetUnderCursor>> = Mutex::new(None);

fn store_target_under_cursor(target: Option<TargetUnderCursor>) {
    *LAST_TARGET_UNDER_CURSOR
        .lock()
...
#[tauri::command]
#[specta::specta]
pub async fn get_target_under_cursor() -> Option<TargetUnderCursor> {
    LAST_TARGET_UNDER_CURSOR
        .lock()
...
// Hand-written rather than derived: `LogicalBounds` comes from scap-targets and
// doesn't implement PartialEq, and it isn't worth changing a shared crate for
// this. Used to skip re-emitting an unchanged cursor target (see the poll loop
// in `open_target_select_overlays`).
impl PartialEq for WindowUnderCursor {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id
            && self.app_name == other.app_name
            && self.bounds.position().x() == other.bounds.position().x()
            && self.bounds.position().y() == other.bounds.position().y()
            && self.bounds.size().width() == other.bounds.size().width()
            && self.bounds.size().height() == other.bounds.size().height()
    }
}
...
pub async fn open_target_select_overlays(
    ...
    // the previous one.
    store_target_under_cursor(None);
    ...
    for (id, window) in app.webview_windows() {
        if let Ok(WindowId::TargetSelectOverlay {
    ...
    for display_id in &display_ids {
        ...
        let existing_window = WindowId::TargetSelectOverlay {
        ...
        if let Some(window) = existing_window {
            ...
            if let Ok(window) = (ShowQuiroWindow::TargetSelectOverlay {
                display_id: display_id.clone(),
                target_mode,
            })
            .show(&app)
    ...
    let focus_window = WindowId::TargetSelectOverlay {
    ...
    let handle = tokio::spawn({
        ...
        async move {
            // The cursor is polled at 20Hz, but it's stationary most of that
            // time. Emitting unconditionally would push 20 IPC messages/sec to
            // *every* overlay window (one per display) and re-render each of
            // them, for identical data. Only emit on an actual change.
            let mut last_emitted: Option<TargetUnderCursor> = None;
            ...
            while let Some((display, window)) = read_target_under_cursor(
                ...
                // Mirrored every tick (cheap, in-process) even when we skip the
                // emit, so `get_target_under_cursor` is always current for
                // overlays that mount mid-stream.
                store_target_under_cursor(Some(payload.clone()));

                if last_emitted.as_ref() != Some(&payload) {
                    let _ = payload.clone().emit(&app);
...
pub fn close_target_select_overlay_windows(app: &AppHandle) {
    ...
    for (id, window) in app.webview_windows() {
        if let Ok(WindowId::TargetSelectOverlay { display_id }) = WindowId::from_str(&id) {
            saw_overlay = true;
    ...
    pub fn spawn(&self, id: &DisplayId, window: WebviewWindow) {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                let quiro_main = WindowId::Main.get(app);
                let quiro_settings = WindowId::Settings.get(app);

                let main_window_available = quiro_main.is_some();
                let settings_window_available = quiro_settings.is_some();
                ...
                if window.is_visible().unwrap_or(false)
                    && let Some(quiro_main) = quiro_main
                {
