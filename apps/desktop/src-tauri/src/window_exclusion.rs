...
pub fn append_matching_webview_window_ids(
    ...
    for (label, window) in app.webview_windows() {
        let Ok(window_id) = WindowId::from_str(&label) else {
            continue;
        };
...
mod tests {
    ...
    fn matches_by_window_title() {
        let exclusion = title_exclusion("Quiro Camera");
        assert!(exclusion.matches(None, None, Some("Quiro Camera")));
    ...
    fn matches_by_owner_name() {
        let exclusion = owner_exclusion("Quiro");
        assert!(exclusion.matches(None, Some("Quiro"), None));
    ...
    fn matches_owner_and_title_requires_both() {
        let exclusion = WindowExclusion {
            bundle_identifier: None,
            owner_name: Some("Quiro".to_string()),
            window_title: Some("Quiro Camera".to_string()),
        };
        assert!(exclusion.matches(None, Some("Quiro"), Some("Quiro Camera")));
        assert!(!exclusion.matches(None, Some("Quiro"), Some("Wrong Title")));
        assert!(!exclusion.matches(None, Some("Wrong Owner"), Some("Quiro Camera")));
        assert!(!exclusion.matches(None, None, Some("Quiro Camera")));
    }
    ...
    fn instant_mode_removes_camera_exclusion() {
        let exclusions = vec![
            title_exclusion("Quiro"),
            title_exclusion("Quiro Camera"),
            title_exclusion("Quiro Settings"),
            title_exclusion("Quiro Recording Controls"),
        ];

        let filtered = filter_for_instant_mode(exclusions, "Quiro Camera");

        assert_eq!(filtered.len(), 3);
        assert!(
            filtered
                .iter()
                .all(|e| e.window_title.as_deref() != Some("Quiro Camera"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Quiro"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Quiro Settings"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Quiro Recording Controls"))
        );
    }

    #[test]
    fn instant_mode_noop_when_camera_absent() {
        let exclusions = vec![title_exclusion("Quiro"), title_exclusion("Quiro Settings")];
    ...
    fn instant_mode_handles_empty_list() {
        let filtered = filter_for_instant_mode(vec![], "Quiro Camera");
        assert!(filtered.is_empty());
    }

    #[test]
    fn matches_webview_title_using_exclusion_rules() {
        let exclusions = vec![title_exclusion("Quiro Camera")];

        assert!(matches_window_title(&exclusions, "Quiro Camera"));
        assert!(!matches_window_title(&exclusions, "Quiro Recording Controls"));
    }
    ...
    fn default_exclusions_contain_camera() {
        ...
        assert!(
            defaults
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Quiro Camera")),
            "Default exclusions must include 'Quiro Camera' — instant mode filtering depends on this"
        );
    }
    ...
    fn default_exclusions_only_match_camera_by_title() {
        ...
        let camera = defaults
            .iter()
            .find(|e| e.window_title.as_deref() == Some("Quiro Camera"))
            .expect("Quiro Camera must be in default exclusions");
