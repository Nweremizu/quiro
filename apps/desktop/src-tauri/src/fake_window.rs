...
// Geometry of the in-progress recording controls. WIDTH/HEIGHT are the
// transparent window; the bar itself is a horizontal pill centred inside it,
// sized to its own content, with the slack around it left for the shadow.
// ToolbarWindow.tsx pins the bar to exactly BAR_HEIGHT and the placement math
// below measures back from it, so the two sides have to agree.
pub(crate) const RECORDING_CONTROLS_WIDTH: f64 = 320.0;
pub(crate) const RECORDING_CONTROLS_HEIGHT: f64 = 96.0;
const RECORDING_CONTROLS_BAR_HEIGHT: f64 = 44.0;
const RECORDING_CONTROLS_BOTTOM_PADDING: f64 = 26.0;
            ...
            if is_recording_controls {
                let capture_target = app.state::<ArcLock<App>>().try_read().ok().and_then(|s| {
                    match &s.recording_state {
                        ...
                        RecordingState::Active { target } => Some(target.clone()),
