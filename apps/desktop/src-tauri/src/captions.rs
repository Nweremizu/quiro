...
pub fn parse_captions_json(json: &str) -> Result<quiro_project::CaptionsData, String> {
            ...
            if let Some(segments_array) = json_value.get("segments").and_then(|v| v.as_array()) {
                ...
                for segment in segments_array {
                    if let (Some(id), Some(start), Some(end), Some(text)) = (
                        ...
                        if let Some(words_array) = segment.get("words").and_then(|v| v.as_array()) {
                            for word in words_array {
                                if let (Some(w_text), Some(w_start), Some(w_end)) = (
                                    ...
                                    words.push(quiro_project::CaptionWord {
                        ...
                        segments.push(quiro_project::CaptionSegment {
                            id: id.to_string(),
                            start: start as f32,
                            end: end as f32,
                            text: text.to_string(),
                ...
                let settings = if let Some(settings_obj) = json_value.get("settings") {
                    ...
                    quiro_project::CaptionSettings::default()
                };

                Ok(quiro_project::CaptionsData {
                    segments,
                    settings,
                    ..Default::default()
                })
    ...
    match parse_captions_json(&json) {
        Ok(project_captions) => {
            ...
            let tauri_captions = CaptionData {
                ...
                settings: project_captions.settings,
                source_timed: project_captions.source_timed,
            };
...
pub async fn download_whisper_model(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        .await;
        if result.is_err() {
            let _ = tokio::fs::remove_file(&validated_path).await;
            let _ = tokio::fs::remove_file(validated_path.with_extension("download")).await;
        }
...
pub async fn export_captions_srt(
    ...
    let captions_with_settings = CaptionData {
        ...
        source_timed: captions.source_timed,
    };
