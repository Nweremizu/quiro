...
impl From<quiro_recording::StudioQuality> for StudioRecordingQuality {
    fn from(value: quiro_recording::StudioQuality) -> Self {
        match value {
            quiro_recording::StudioQuality::Compatibility => Self::Compatibility,
            quiro_recording::StudioQuality::Balanced => Self::Balanced,
            quiro_recording::StudioQuality::Ultra => Self::Ultra,
        }
    }
}

impl From<StudioRecordingQuality> for quiro_recording::StudioQuality {
...
pub fn default_studio_recording_quality() -> StudioRecordingQuality {
    quiro_recording::default_studio_recording_quality().into()
}
...
// NOTE: Do not add "Quiro Target Select" here — on Windows, WDA_EXCLUDEFROMCAPTURE applied to that
// hidden window causes it to reappear as a ghost overlay after recording ends.
const DEFAULT_EXCLUDED_WINDOW_TITLES: &[&str] = &[
    "Quiro",
    "Quiro Settings",
    "Quiro Screenshot Editor",
    "Quiro Recording Controls",
    "Quiro Camera",
    "Quiro Window Capture Occluder",
    "Quiro Capture Area",
    "Quiro Mode Selection",
    "Quiro Recordings Overlay",
    "Quiro Teleprompter",
];
...
fn default_instant_mode_max_resolution() -> u32 {
    quiro_recording::DEFAULT_INSTANT_MODE_MAX_RESOLUTION
}

fn default_max_fps() -> u32 {
    quiro_recording::DEFAULT_STUDIO_MAX_FPS
}

fn default_custom_cursor_capture() -> bool {
    quiro_recording::DEFAULT_CUSTOM_CURSOR_CAPTURE
}

fn default_capture_keyboard_events() -> bool {
    quiro_recording::DEFAULT_CAPTURE_KEYBOARD_EVENTS
}

fn default_crash_recovery_recording() -> bool {
    quiro_recording::DEFAULT_CRASH_RECOVERY_RECORDING
}
...
impl Default for GeneralSettingsStore {
    fn default() -> Self {
        Self {
            ...
            custom_cursor_capture: quiro_recording::DEFAULT_CUSTOM_CURSOR_CAPTURE,
...
pub fn init(app: &AppHandle) {
    ...
    const REMOVE_TARGET_SELECT_MIGRATION_KEY: &str = "remove_quiro_target_select_exclusion_v1";
    if let Ok(raw_store) = app.store("store")
        && raw_store.get(REMOVE_TARGET_SELECT_MIGRATION_KEY).is_none()
    {
        store
            .excluded_windows
            .retain(|w| w.window_title.as_deref() != Some("Quiro Target Select"));
...
fn register_bundled_muxer_binary(_app: &AppHandle) {
    if std::env::var_os(quiro_recording::oop_muxer::ENV_BIN_PATH).is_some() {
        return;
    }
        ...
        && let Some(dir) = exe.parent()
    {
        ...
        if candidate.is_file() {
            match quiro_recording::oop_muxer::set_muxer_binary_override(candidate.clone()) {
                Ok(()) => {
                    tracing::info!(
                        path = %candidate.display(),
                        "Registered executable-adjacent quiro-muxer binary for out-of-process muxer"
                    );
                }
                Err(existing) => {
                    tracing::debug!(
                        ...
                        "quiro-muxer override already registered; keeping existing"
                    );
                }
            }
        }
