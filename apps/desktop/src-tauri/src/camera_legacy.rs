...
fn scaled_preview_dimensions(width: u32, height: u32, state: &CameraPreviewState) -> (u32, u32) {
    let blur_enabled = state.background_blur != quiro_project::BackgroundBlurMode::Off;
...
struct WsBlurResources {
    device: wgpu::Device,
    queue: wgpu::Queue,
    processor: quiro_camera_effects::BlurProcessor,
...
impl WsBlurState {
    ...
    fn process(
        ...
        // this machine.
        if is_low_spec_preview() || quiro_camera_effects::blur_disabled() {
            return None;
        }
...
fn init_headless_blur() -> Option<WsBlurResources> {
    ...
    let instance = quiro_rendering::create_wgpu_instance_sync();
    ...
    let mut processor =
        quiro_camera_effects::BlurProcessor::new(&device, wgpu::TextureFormat::Rgba8Unorm).ok()?;
