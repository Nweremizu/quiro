...
#[cfg(target_os = "macos")]
use crate::{general_settings::GeneralSettingsStore, windows::WindowId};
#[cfg(target_os = "macos")]
use cidre::{av, sc};
#[cfg(target_os = "macos")]
...
pub async fn request_permission(_app: tauri::AppHandle, _permission: OSPermission) {
    ...
    // ponytail: no tray icon module yet — Cap refreshes its tray menu here
    // (permission state affects what it shows). Add back once Quiro has one.
}

#[derive(Serialize, Deserialize, Debug, specta::Type, Clone, PartialEq)]
#[serde(rename_all = "camelCase")]
