...
import { canShowFloatingWebcamPreview } from "@/components/launch/floating-webcam-preview";
import {
  isWebcamPreviewShape,
...
const WEBCAM_SHAPE_STORAGE_KEY = "quiro:webcam-preview-shape";
...
export function useWebcamPreviewOverlay({
  ...
  const [showFloatingWebcamPreview, setShowFloatingWebcamPreview] =
    useState(true);
  const [webcamPreviewShape, setWebcamPreviewShape] =
    useState<WebcamPreviewShape>("rounded-square");
  ...
  const previewDragPendingPointerRef = useRef<{
    clientX: number;
    clientY: number;
  } | null>(null);
  ...
  useEffect(() => {
    if (!webcamEnabled) {
      ...
      if (recordingWebcamPreviewContainerRef.current) {
        recordingWebcamPreviewContainerRef.current.style.transform =
          "translate(0px, 0px)";
      }
  ...
  const handleWebcamPreviewPointerMove = useCallback(
    (event: PointerEvent<HTMLDivElement>) => {
      ...
      previewDragMoveRafRef.current = requestAnimationFrame(() => {
        ...
        const nextOffset = {
          x:
            latestDragState.originX +
            (clampedLeft - latestDragState.initialLeft),
          y:
            latestDragState.originY + (clampedTop - latestDragState.initialTop),
        };
  ...
  useEffect(() => {
    ...
    try {
      const stored = window.localStorage.getItem(WEBCAM_SHAPE_STORAGE_KEY);
      if (stored && isWebcamPreviewShape(stored)) {
        setWebcamPreviewShape(stored);
      }
    } catch {
      // Ignore storage failures and keep the default shape.
    }
  }, []);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(WEBCAM_SHAPE_STORAGE_KEY, webcamPreviewShape);
    } catch {
