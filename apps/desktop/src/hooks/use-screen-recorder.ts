...
const MICROPHONE_FALLBACK_ERROR_TOAST_ID =
  "recording-microphone-fallback-error";
...
type UseScreenRecorderReturn = {
  ...
  recordingAspectRatio: number | null;
};
...
export function useScreenRecorder(): UseScreenRecorderReturn {
  ...
  const [recordingAspectRatio, setRecordingAspectRatio] = useState<
    number | null
  >(null);
  ...
  const preparePermissions = useCallback(
    async (options: { startup?: boolean } = {}) => {
      ...
      if (!screenPermission.success || screenPermission.status !== "granted") {
        ...
        alert(
          options.startup
            ? "Quiro needs Screen Recording permission before you start. System Settings has been opened. After enabling it, quit and reopen Quiro."
            : "Screen Recording permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Quiro before recording.",
        );
        return false;
      }
      ...
      alert(
        options.startup
          ? "Quiro also needs Accessibility permission for cursor tracking. System Settings has been opened. After enabling it, quit and reopen Quiro."
          : "Accessibility permission is still missing. System Settings has been opened again. Enable it, then quit and reopen Quiro before recording.",
      );
  ...
  const startRecording = async () => {
    ...
    try {
        ...
        if (microphoneEnabled) {
          try {
            ...
            const mic = devices.find(
              (d) =>
                d.deviceId === microphoneDeviceId && d.kind === "audioinput",
            );
      ...
      if (width > 0 && height > 0) {
        setRecordingAspectRatio(width / height);
      }
