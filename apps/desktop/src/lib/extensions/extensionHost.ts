...
import {
  createExtensionModuleUrl,
  resolveExtensionRelativeFileUrl,
} from "./fileUrls";
import type {
  ...
  FrameInstance,
  QuiroExtensionAPI,
  QuiroExtensionModule,
  RenderHookContext,
  RenderHookFn,
  RenderHookPhase,
} from "./types";

const EXTENSION_SETTINGS_STORAGE_KEY = "quiro.extension-settings.v1";
...
export class ExtensionHost {
  ...
  private _keystrokeEvents: Array<{
    timeMs: number;
    key: string;
    modifiers: string[];
  }> = [];
  ...
  async activateExtension(
    ...
    let mod: QuiroExtensionModule | null = null;
    try {
      ...
      try {
        const loaded: QuiroExtensionModule = await import(
      ...
      console.log(
        `[extensions] Activated: ${info.manifest.name} v${info.manifest.version}`,
      );
    } catch (err) {
      ...
      this.notifyListeners();
      console.error(
        `[extensions] Failed to activate ${info.manifest.id}:`,
        err,
      );
      throw err;
    }
  ...
  async deactivateExtension(extensionId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `[extensions] Error during deactivate of ${extensionId}:`,
        err,
      );
    }
  ...
  executeCursorEffects(context: CursorEffectContext): boolean {
    ...
    for (const effect of this.cursorEffects) {
      ...
      try {
        ...
        console.warn(
          `[extensions] Cursor effect error (${effect.extensionId}):`,
          err,
        );
      } finally {
        context.ctx.restore();
  ...
  private writePersistedSettingsStore(
    ...
    return {
      ...
      registerFrame(frame: ContributedFrame): () => void {
        ...
        if (frame.draw) {
          ...
          throw new Error(
            "Device frame must provide either draw, file, or dataUrl",
          );
        }
      ...
      registerWallpaper(wallpaper: ContributedWallpaper): () => void {
        ...
        const resolvedUrl = resolveExtensionRelativeFileUrl(
          extensionPath,
          wallpaper.file,
        );
      ...
      getCursorAt(timeMs: number) {
        ...
        if (timeMs >= t[t.length - 1].timeMs)
          return { ...t[t.length - 1], timeMs };
      ...
      onSettingChange(
        callback: (settingId: string, value: unknown) => void,
      ): () => void {
  ...
  async syncConfiguredExtensions(discovered: ExtensionInfo[]): Promise<void> {
    ...
    for (const ext of discovered) {
      if (
        ext.status !== "active" ||
        this.activeExtensions.has(ext.manifest.id)
      ) {
        continue;
      }
