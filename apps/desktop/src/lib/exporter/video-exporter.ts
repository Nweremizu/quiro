...
import type { SupportedMp4EncoderPath } from "./mp4-support";
...
export class VideoExporter {
  ...
  async export(): Promise<ExportResult> {
    try {
      ...
      const shouldUseExperimentalNativeExport =
        this.shouldUseExperimentalNativeExport();
      ...
      const hasSourceAudioFallback =
        (this.config.sourceAudioFallbackPaths ?? []).length > 0;
      const hasAudio =
        videoInfo.hasAudio || hasAudioRegions || hasSourceAudioFallback;
      ...
      if (useNativeEncoder && nativeAudioPlan) {
        ...
        const result = await this.finishNativeVideoExport(
          nativeAudioPlan,
          totalFrames,
        );
  ...
  private async tryStartNativeVideoExport(): Promise<boolean> {
    ...
    try {
      ...
      console.warn(
        "[VideoExporter] Native encoder support check failed:",
        error,
      );
      return false;
    }
    ...
    const encoder = new VideoEncoder({
      output: (chunk) => {
        ...
        const writePromise = this.nativePendingWrite
          .then(async () => {
            const writeResult =
              await window.electronAPI.nativeVideoExportWriteFrame(
                sessionId,
  ...
  private async encodeRenderedFrameNative(
    ...
    while (
      this.nativeWritePromises.size >= this.maxNativeWriteInFlight &&
      !this.cancelled
    ) {
    ...
    while (
      ...
      Math.max(
        1,
        Math.floor(this.config.maxEncodeQueue ?? DEFAULT_MAX_ENCODE_QUEUE),
      )
    ) {
  ...
  private async initializeEncoder(): Promise<void> {
    ...
    for (const candidateCodec of CODEC_FALLBACK_LIST) {
      ...
      if (swSupport.supported) {
        resolvedCodec = candidateCodec;
        console.log(
          `[VideoExporter] Using software encoding with codec ${candidateCodec}`,
        );
  ...
  private cleanup(): void {
    ...
    if (this.nativeExportSessionId) {
      if (typeof window !== "undefined") {
        void window.electronAPI?.nativeVideoExportCancel?.(
