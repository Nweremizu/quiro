import type { AudioRegion, SpeedRegion, TrimRegion } from "@/types/editor";

const MIN_FILTERGRAPH_SPEED = 0.5;
const MAX_FILTERGRAPH_SPEED = 2;

export type EditedTrackStrategy =
...
function isSafeFiltergraphSpeed(speed: number): boolean {
  return (
    ...
    speed >= MIN_FILTERGRAPH_SPEED &&
    speed <= MAX_FILTERGRAPH_SPEED
  );
}

function hasFiniteTimelineRange(
  startMs: number,
  endMs: number,
  sourceDurationMs: number,
): boolean {
...
function buildKeptRanges(
  ...
  const sortedTrimRegions = [...trimRegions].sort(
...
export function classifyEditedTrackStrategy(
  input: EditedTrackStrategyInput,
): EditedTrackStrategy {
  ...
  if (
    !hasSafeFiltergraphSpeedRegions(input.speedRegions, input.sourceDurationMs)
  ) {
    return "offline-render-fallback";
  }
