...
import { buildActiveCaptionLayout } from "@/components/editor/utils/caption-layout";
import {
  CAPTION_FONT_WEIGHT,
  CAPTION_LINE_HEIGHT,
  getCaptionPadding,
...
} from "@/components/editor/utils/caption-style";
import type {
  AnnotationRegion,
  AutoCaptionSettings,
  CaptionCue,
...
import { getDefaultCaptionFontFamily, ZOOM_DEPTH_SCALES } from "@/types/editor";
...
import {
  ...
  PixiCursorOverlay,
  preloadCursorAssets,
} from "@/components/editor/videoPlayback/cursorRenderer";
...
import { ForwardFrameSource } from "./forward-frame-source";
import { resolveMediaElementSource } from "./local-media-source";
import {
...
type ResolvedCaptionLayout = NonNullable<
  ReturnType<typeof buildActiveCaptionLayout>
>;
...
const CANVAS_RENDERER_NOT_IMPLEMENTED_HINT =
  "CanvasRenderer is not yet implemented";
...
function isCanvasRenderer(application: Application): boolean {
  ...
  return Boolean(
    rendererName &&
    (rendererName.includes("canvasrenderer") ||
      rendererName.includes("canvas")),
  );
}
...
function toErrorMessage(error: unknown): string {
  return error instanceof Error
    ? error.message
    : String(error ?? "Unknown renderer init error");
}

function summarizeRendererAttempts(
  attempts: readonly PixiRendererAttempt[],
): string {
  const details = attempts
    .map((attempt) => `${attempt.backend}: ${attempt.message}`)
    .join(" | ");
...
function applyCoverLayoutToSprite(
  ...
  const coverScale = Math.max(
    targetWidth / safeSourceWidth,
    targetHeight / safeSourceHeight,
  );
...
export class FrameRenderer {
  ...
  private backgroundVideoFrameStagingCtx: CanvasRenderingContext2D | null =
    null;
  ...
  async initialize(): Promise<void> {
    ...
    this.annotationAssets = await preloadAnnotationAssets(
    ...
    console.log(
      `[FrameRenderer] Export renderer backend: ${this.rendererBackend}`,
    );
  }
  ...
  private ensureShadowLayerCanvas(
    layer: ShadowLayer,
    width: number,
    height: number,
  ): void {
    ...
    layer.canvas.width = targetWidth;
    layer.canvas.height = targetHeight;
    layer.context = configureHighQuality2DContext(
    ...
    if (layer.sprite) {
      ...
      layer.textureSource =
        nextTexture.source as unknown as MutableVideoTextureSource;
  ...
  private rasterizeShadowLayer(
    ...
    layer.context.filter =
      options.blur > 0 ? `blur(${options.blur}px)` : "none";
  ...
  private closeRetainedVideoFrame(
    kind: "scene" | "background" | "webcam",
  ): void {
  ...
  private async resolveDetachedVideoFrameSource(
    ...
    const cachedBitmap =
      kind === "scene"
        ? this.retainedSceneBitmap
        : this.retainedBackgroundBitmap;
      ...
      if (
        ...
        if (!gradientMatch) {
          ...
          parts.forEach((part, index) => {
            if (
              type === "linear" &&
              (part.startsWith("to ") || part.includes("deg"))
            ) {
              return;
            }
  ...
  private async ensureBackgroundSprite(
    ...
    if (!this.backgroundSprite) {
      ...
      this.backgroundTextureSource =
        texture.source as unknown as MutableVideoTextureSource;
      this.backgroundContainer.addChild(this.backgroundSprite);
      ...
      if (this.config.backgroundBlur > 0) {
        ...
        this.backgroundBlurFilter.resolution =
  ...
  private createPreblurredBackgroundCanvas(
    ...
    const blurredCtx = configureHighQuality2DContext(
      blurredCanvas.getContext("2d"),
    );
    if (!blurredCtx) {
      return null;
    }
  ...
  private drawCaptionOverlay(context: CanvasRenderingContext2D): void {
    ...
    const drawX =
      this.captionSprite.x - drawWidth * this.captionSprite.anchor.x;
    const drawY =
      this.captionSprite.y - drawHeight * this.captionSprite.anchor.y;
  ...
  private async composeBlurAnnotationFrame(
    ...
    context.drawImage(
      sourceCanvas ?? (this.app.canvas as HTMLCanvasElement),
      0,
      0,
    );
  ...
  private buildCaptionRenderState(timeMs: number): CaptionRenderState | null {
    ...
    const layout = buildActiveCaptionLayout({
      ...
      timeMs,
      settings,
      maxWidthPx: getCaptionTextMaxWidth(
        this.config.width,
        settings.maxWidth,
        fontSize,
      ),
    ...
    const centerY =
      this.config.height -
      (this.config.height * settings.bottomOffset) / 100 -
      boxHeight / 2;
  ...
  private ensureCaptionCanvas(width: number, height: number): void {
    ...
    this.captionCanvas.width = targetWidth;
    this.captionCanvas.height = targetHeight;
    this.captionCtx = configureHighQuality2DContext(
      this.captionCanvas.getContext("2d"),
    );
  ...
  private rasterizeCaptionSprite(state: CaptionRenderState): void {
    ...
    state.layout.visibleLines.forEach((line, lineIndex) => {
      ...
      const lineY =
        padding.y + state.lineHeight * lineIndex + state.lineHeight / 2;
  ...
  private updateCaptionLayer(timeMs: number): void {
    ...
    this.captionSprite.position.set(
      state.centerX,
      state.centerY + state.layout.translateY,
    );
  ...
  private async syncBackgroundFrame(timeSeconds: number): Promise<void> {
    if (this.backgroundForwardFrameSource) {
      ...
      if (
        ...
        this.backgroundForwardFrameDurationSec =
          resolvedDecodedDuration ?? null;
        this.closeBackgroundDecodedFrame();
        decodedFrame?.close();
        try {
          await this.restartBackgroundForwardFrameSource();
          normalizedTargetTime =
            ((timeSeconds % resolvedDecodedDuration!) +
  ...
  private async setupWebcamSource(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      console.warn(
        "[FrameRenderer] Webcam overlay unavailable during export:",
        error,
      );
      this.webcamVideoElement = null;
    });
  ...
  private ensureWebcamSprite(
    ...
    const resolvedSource =
      typeof VideoFrame !== "undefined" && source instanceof VideoFrame
        ? this.stageVideoFrameForTexture(
            source,
            "webcam",
            sourceWidth,
            sourceHeight,
    ...
    if (!this.webcamSprite) {
      ...
      this.webcamTextureSource = this.replaceSpriteTexture(
        this.webcamSprite,
        resolvedSource,
      );
      this.webcamTextureUsesStartupStaging = usesStartupStaging;
  ...
  private setWebcamRenderMode(nextMode: "hidden" | "live" | "cached"): void {
    ...
    if (nextMode === "cached") {
      console.log(
        "[FrameRenderer] Webcam export source fell back to the last synced frame",
      );
  ...
  private shouldRefreshWebcamFrameCache(
    width: number,
    height: number,
  ): boolean {
    ...
    return (
      Math.abs(this.currentVideoTime - this.lastWebcamCacheRefreshTime) >= 0.25
    );
  }
  ...
  private refreshWebcamFrameCache(
    ...
    const sourceRect = getWebcamCropSourceRect(
      this.config.webcam?.cropRegion,
      width,
      height,
    );
  ...
  private resolveRenderableWebcamSource(
    ...
    if (
      ...
      if (
        this.shouldRefreshWebcamFrameCache(liveSourceWidth, liveSourceHeight)
      ) {
  ...
  private applyWebcamLayout(nextLayout: WebcamLayoutCache): void {
    ...
    this.webcamRootContainer.position.set(
      nextLayout.positionX,
      nextLayout.positionY,
    );
  ...
  private updateWebcamOverlay(): void {
    ...
    const canUseLiveSource =
      ...
      (!activeWebcamVideoElement ||
        (activeWebcamVideoElement.readyState >=
  ...
  private async renderSceneSample(
    ...
    const annotationContainerVisible =
  ...
  private async renderTemporalMotionBlurFrame(
    ...
    const blurConfig = getTemporalMotionBlurConfig(
      this.config.zoomTemporalMotionBlur,
      {
    ...
    for (const { offsetUs: sampleOffsetUs, weight } of samplePlan) {
      ...
      const sampleCursorTimestamp = Math.max(
        0,
        cursorTimestamp + sampleOffsetUs,
      );
      ...
      compositeState.context.drawImage(
  ...
  async renderFrame(
    ...
    if (temporalSnapshot) {
      const sourceCanvas =
        ...
        this.ensureTemporalCompositeCanvas()?.canvas;
  ...
  private updateAnimationState(timeMs: number): number {
    ...
    return Math.max(
      ...
      Math.abs(state.x - previousX) /
        Math.max(1, this.layoutCache.stageSize.width),
      Math.abs(state.y - previousY) /
  ...
  destroy(): void {
    ...
    for (const texture of texturesToDestroy) {
      try {
        ...
        console.warn(
          "[FrameRenderer] Failed to destroy texture during cleanup:",
          error,
        );
      }
    }
