...
import { renderCaptions } from "./caption-renderer";
...
export class FrameRenderer {
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.temporalAccumulationCtx) {
      throw new Error(
        "Failed to get 2D context for temporal accumulation canvas",
      );
    }
  ...
  private async setupFrame(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) => {
      img.onload = () => resolve();
      img.onerror = () =>
        reject(new Error(`Failed to load device frame image: ${frameId}`));
      ...
      const hookParams = {
        ...
        cursor: smoothedCursor
          ? {
              ...
              cy: smoothedCursor.cy,
              interactionType:
                this.getCursorPosition(cursorTimeMs)?.interactionType,
            }
  ...
  private async renderTemporalMotionBlurFrame(
    ...
    this.temporalAccumulationCtx.clearRect(
      0,
      0,
      this.config.width,
      this.config.height,
  ...
  private drawFrame(): void {
    if (
      (!this.frameImage && !this.frameDraw) ||
      !this.compositeCtx ||
      !this.layoutCache
    )
      return;
  ...
  private drawWebcamOverlay(
    ...
    const hasLiveWebcamFrame = webcamDecodedFrame
      ? webcamDecodedFrame.displayWidth > 0 &&
        webcamDecodedFrame.displayHeight > 0
      : Boolean(
          webcamVideo &&
          webcamVideo.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA &&
    ...
    const bubbleCtx =
      this.webcamBubbleCtx ??
      configureHighQuality2DContext(bubbleCanvas.getContext("2d"));
    if (!bubbleCtx) {
      return;
    }
    ...
    if (canRefreshCache) {
      const liveFrameWidth =
        webcamDecodedFrame?.displayWidth ?? webcamVideo?.videoWidth ?? 0;
