...
import { DEFAULT_FOCUS } from "@/components/editor/videoPlayback/constants";
import {
  computeCursorFollowFocus,
  createCursorFollowCameraState,
  SNAP_TO_EDGES_RATIO_AUTO,
} from "@/components/editor/videoPlayback/cursorFollowCamera";
import { buildNativeCursorAtlas } from "@/components/editor/videoPlayback/cursorRenderer";
import { computePaddedLayout } from "@/components/editor/videoPlayback/layoutUtils";
import {
  createSpringState,
  getZoomSpringConfig,
  resetSpringState,
...
} from "@/components/editor/videoPlayback/zoomTransform";
import {
...
import { AudioProcessor, isAacAudioEncodingSupported } from "./audio-encoder";
import {
...
} from "./finalization-timeout";
import { getLocalFilePath } from "./local-media-source";
import { FrameRenderer as ModernFrameRenderer } from "./modern-frame-renderer";
import {
  getOrderedSupportedMp4EncoderCandidates,
  type SupportedMp4EncoderPath,
} from "./mp4-support";
import { VideoMuxer } from "./muxer";
import { roundNativeStaticLayoutContentSize } from "./native-static-layout-geometry";
import { buildNativeStaticLayoutCursorTelemetry } from "./native-static-layout-telemetry";
import {
...
export class ModernVideoExporter {
  ...
  private activeFinalizationProgressWatchdog: FinalizationProgressWatchdog | null =
    null;
  private lastFinalizationRenderProgress =
    INITIAL_FINALIZATION_PROGRESS_STATE.lastRenderProgress;
  private lastFinalizationAudioProgress =
    INITIAL_FINALIZATION_PROGRESS_STATE.lastAudioProgress;
  ...
  async export(): Promise<ExportResult> {
    try {
      ...
      this.backpressureProfile = getExportBackpressureProfile({
        encodeBackend:
          shouldDeferNativeEncoderStart || useNativeEncoder
            ? "ffmpeg"
            : "webcodecs",
      ...
      console.log("[VideoExporter] Backpressure profile", {
        ...
        maxPendingFrames:
          this.config.maxPendingFrames ??
          this.backpressureProfile.maxPendingFrames,
      ...
      if (
        ...
        if (nativeVideoInfo) {
          ...
          const totalFrames = Math.ceil(
            effectiveDuration * this.config.frameRate,
          );
      ...
      await this.streamingDecoder.decodeAll(
        ...
        async (
          ...
          if (useNativeEncoder) {
            ...
            await this.encodeRenderedFrame(
              timestamp,
              frameDuration,
              frameIndex,
            );
      ...
      if (useNativeEncoder) {
        ...
        if (this.nativeH264Encoder) {
          await this.measureFinalizationStage(
            "nativeEncoderFlushMs",
            async () => {
        ...
        const finishResult =
          await this.finishNativeVideoExport(nativeAudioPlan);
        ...
        if (
          ...
          return {
            ...
            error:
              finishResult.error ||
              `${NATIVE_EXPORT_ENGINE_NAME} export failed`,
            metrics: this.buildExportMetrics(),
          };
        }
  ...
  private getRuntimePlatform() {
    ...
    return normalizeLightningRuntimePlatform(
  ...
  private getLightningErrorGuidance(message: string): string[] {
    ...
    if (
      ...
      guidance.add(
        "Try Good or Medium quality to reduce output resolution and bitrate.",
      );
      guidance.add(
  ...
  private buildLightningExportError(error: unknown): string {
    ...
    if (
      this.lastNativeExportError &&
      !message.includes(this.lastNativeExportError)
    ) {
      lines.push(
        `${NATIVE_EXPORT_ENGINE_NAME} fallback: ${this.lastNativeExportError}`,
      );
    }
  ...
  private getNativeWebcamSourcePath(): string | null {
    const source =
      this.config.webcam?.sourcePath || this.config.webcamUrl || "";
  ...
  private async loadNativeStaticLayoutVideoInfo(): Promise<DecodedVideoInfo | null> {
    if (
      ...
      !window.electronAPI?.probeNativeVideoMetadata
    ) {
      return null;
    }
    ...
    try {
      const result =
        await window.electronAPI.probeNativeVideoMetadata(sourcePath);
  ...
  private buildNativeTrimSegments(
    durationMs: number,
  ...
  private getNativeStaticLayoutEffectiveDuration(
    ...
    if (speedRegions.length > 0) {
      const timelineSegments =
        this.buildNativeStaticLayoutSourceSegments(sourceDurationMs);
    ...
    return trimSegments.reduce(
      (totalSec, segment) =>
        totalSec + (segment.endMs - segment.startMs) / 1000,
      0,
    );
  }
  ...
  private buildNativeAudioPlan(videoInfo: DecodedVideoInfo): NativeAudioPlan {
    ...
    const hasTimedSourceAudioFallback = sourceAudioFallbackPaths.some(
      (audioPath) =>
        (this.config.sourceAudioFallbackStartDelayMsByPath?.[audioPath] ?? 0) >
        0,
    );
    ...
    const usesEmbeddedPrimaryAudio =
      ...
      primaryAudioSourcePath === localVideoSourcePath;
    ...
    const primaryAudioSourceCodec = usesEmbeddedPrimaryAudio
  ...
  private getNativeStaticLayoutSourceCrop(videoInfo: DecodedVideoInfo) {
    ...
    const left =
      Math.min(sourceWidth - 2, Math.max(0, Math.floor(cropX * sourceWidth))) &
      ~1;
    const top =
      Math.min(
        sourceHeight - 2,
    ...
    const right = Math.min(
      sourceWidth,
      Math.max(left + 2, Math.ceil(cropRight * sourceWidth)),
    );
  ...
  private canUseNativeStaticTailTrim(
    ...
    const outputDurationMs = Math.max(
      0,
      Math.round(effectiveDurationSec * 1000),
    );
  ...
  private getNativeStaticLayoutSkipReason(
    ...
    return (
      this.getNativeStaticLayoutSkipReasons(
        audioPlan,
        videoInfo,
        effectiveDurationSec,
      )[0] ?? null
  ...
  private async resolveNativeStaticLayoutBackground(): Promise<NativeStaticLayoutBackground | null> {
    ...
    if (/^#?[0-9a-f]{6}$/i.test(wallpaper)) {
      return {
        backgroundColor: wallpaper.startsWith("#")
          ? wallpaper
          : `#${wallpaper}`,
        backgroundImagePath: null,
      };
    ...
    if (wallpaper.startsWith("data:image/") || wallpaper.startsWith("blob:")) {
      const materialized =
        await this.materializeNativeStaticLayoutImageSource(wallpaper);
      if (materialized) {
        return materialized;
      }
    ...
    if (wallpaper.startsWith("data:") || wallpaper.startsWith("blob:")) {
      this.nativeStaticLayoutBackgroundSkipReason =
        "unsupported-background-data-or-blob";
      return null;
    }

    if (wallpaper.startsWith("http")) {
      this.nativeStaticLayoutBackgroundSkipReason =
        "unsupported-background-remote";
      return null;
    }
    ...
    this.nativeStaticLayoutBackgroundSkipReason =
      "unsupported-background-local-path";
    return null;
  }
  ...
  private async materializeNativeStaticLayoutImageSource(
    ...
    try {
      ...
      const mimeType = (
        blob.type || this.getDataUrlMimeType(imageSource)
      ).toLowerCase();
      ...
      console.warn(
        "[VideoExporter] Unable to materialize native background image",
        error,
      );
      return null;
    }
  ...
  private async materializeNativeStaticLayoutGradientBackground(
    ...
    try {
      ...
      console.warn(
        "[VideoExporter] Unable to materialize native gradient background",
        error,
      );
      return null;
    }
  ...
  private createNativeStaticLayoutGradient(
    ...
    const parts = this.splitCssGradientArguments(params).map((part) =>
      part.trim(),
    );
  ...
  private async writeNativeStaticLayoutTempAsset(
    ...
    try {
      ...
      if (!writeResult.success) {
        throw new Error(
          writeResult.error || "Failed to write native background temp file",
        );
      }

      const closeResult = await api.closeExportStream(streamId);
      streamId = undefined;
      return closeResult.success && closeResult.tempPath
        ? closeResult.tempPath
        : null;
    } catch (error) {
      console.warn(
        "[VideoExporter] Unable to write native background temp file",
        error,
      );
      if (streamId) {
        await api
          .closeExportStream(streamId, { abort: true })
          .catch(() => undefined);
      }
      return null;
    }
  ...
  private async resolveNativeBundledAssetPath(
    assetPath: string,
  ): Promise<string | null> {
    ...
    try {
      const result =
        await window.electronAPI?.listAssetDirectory?.(assetDirectory);
      if (
        result?.success &&
        result.files &&
        ...
        console.warn(
          "[VideoExporter] Native static layout wallpaper asset is missing",
          {
            assetPath,
          },
        );
  ...
  private async renderEditedAudioForNativeMux(
    ...
    const audioBlob = await this.measureFinalizationStage(
      "editedAudioRenderMs",
      async () =>
        this.awaitWithFinalizationTimeout(
  ...
  private getNativeStaticLayoutWebcamOverlay(): NativeStaticLayoutWebcamOverlay | null {
    ...
    return {
      ...
      timeOffsetMs: Number.isFinite(webcam.timeOffsetMs)
        ? webcam.timeOffsetMs
        : 0,
    };
  }
  ...
  private getNativeStaticLayoutZoomTelemetry(
    ...
    for (let frameIndex = 0; frameIndex < totalFrames; frameIndex += 1) {
      ...
      const deltaMs =
        lastContentTimeMs !== null
          ? timeMs - lastContentTimeMs
          : frameDurationMs;
  ...
  private async tryExportNativeStaticLayout(
    ...
    const borderRadius = Math.max(
      0,
      (this.config.borderRadius ?? 0) * canvasScaleFactor,
    );
    ...
    const cursorAtlas =
      cursorTelemetry && cursorTelemetry.length > 0
        ? await buildNativeCursorAtlas(
            this.config.cursorStyle ?? "tahoe",
          ).catch((error) => {
            console.warn(
              "[VideoExporter] Native cursor atlas unavailable",
              error,
            );
            return null;
          })
    ...
    const startedAt = this.getNowMs();
    const sessionId = `quiro-static-layout-${Date.now()}-${Math.random()
      .toString(36)
      .slice(2, 8)}`;
    ...
    unsubscribeNativeProgress =
      window.electronAPI.onNativeStaticLayoutExportProgress?.((progress) => {
        ...
        const rawNativeCurrentFrame = Math.max(
          0,
        ...
        const maxExtractingFrame = Math.max(
          ...
          Math.min(
            ...
            Math.floor(
              totalFrames *
                (NATIVE_STATIC_LAYOUT_MAX_EXTRACTING_PROGRESS / 100),
            ),
          ),
        );
    ...
    try {
      const result = await window.electronAPI.nativeStaticLayoutExport({
        ...
        experimentalWindowsGpuCompositor:
          this.config.experimentalNativeExport === true,
        audioOptions: {
          ...audioOptions,
  ...
  private async encodeRenderedFrameNative(
    ...
    if (!this.nativeH264Encoder || !this.nativeExportSessionId) {
      ...
      throw new Error(
        `${NATIVE_EXPORT_ENGINE_NAME} export session is not active`,
      );
    }
  ...
  private async finishNativeVideoExport(
    ...
    console.log(
      `[VideoExporter] Finalizing ${NATIVE_EXPORT_ENGINE_NAME} export`,
      {
        sessionId,
        audioMode: audioPlan.audioMode,
        editedTrackStrategy:
    ...
    const result = await this.measureFinalizationStage(
    ...
    if (!result.success) {
      return {
        ...
        error:
          result.error ||
          `Failed to finalize ${NATIVE_EXPORT_ENGINE_NAME} export`,
      };
    }
    ...
    if (videoSource.mode === "stream") {
      if (!window.electronAPI?.muxExportedVideoAudioFromPath) {
        return {
          success: false,
          error:
            "FFmpeg audio fallback via temp path is unavailable in this environment.",
        };
      }
      const result = await this.measureFinalizationStage(
        "ffmpegAudioMuxMs",
  ...
  private queueNativeWriteChunk(sessionId: string, chunk: Uint8Array): void {
    ...
    if (
      ...
      this.pendingNativeWriteBytes >=
        ModernVideoExporter.NATIVE_WRITE_BATCH_MAX_BYTES
    ) {
      this.flushPendingNativeWriteBatch(sessionId);
    }
  }
    ...
    const writePromise = window.electronAPI
      ...
      .then((writeResult) => {
        if (!writeResult.success && !this.cancelled) {
          throw new Error(
            writeResult.error ||
              "Failed to write H.264 chunks to native encoder",
          );
        }
      })
      .catch((error) => {
  ...
  private reportProgress(
    ...
    const sampleFrameDelta = Math.max(
      currentFrame - this.lastProgressSampleFrame,
      0,
    );
    ...
    const safeRenderProgress =
      phase === "finalizing"
        ? Math.max(0, Math.min(renderProgress ?? 100, 100))
        : undefined;
    const percentage =
      phase === "preparing"
        ? 0
    ...
    if (
      nowMs - this.lastThroughputLogTimeMs >= 1000 ||
    ...
    if (this.config.onProgress) {
      this.config.onProgress({
        ...
        nativeStaticLayoutSkipReason:
          this.nativeStaticLayoutSkipReason ?? undefined,
  ...
  private buildExportMetrics(): ExportMetrics {
    const totalElapsedMs =
      this.totalExportStartTimeMs > 0
        ? this.getNowMs() - this.totalExportStartTimeMs
        : 0;
    const safeFrameCount = Math.max(this.processedFrameCount, 1);
    const hasFinalizationStageMetrics =
      Object.keys(this.finalizationStageMs).length > 0;
    ...
    return {
      ...
      finalizationStageMs: hasFinalizationStageMetrics
      ...
      averageRenderFrameMs:
        this.processedFrameCount > 0
          ? this.renderFrameTimeMs / safeFrameCount
          : undefined,
      averageEncodeWaitMs:
        this.processedFrameCount > 0
          ? this.encodeWaitTimeMs / safeFrameCount
          : undefined,
      averageNativeCaptureMs:
        this.processedFrameCount > 0
      ...
      averageNativeWriteMs:
        this.processedFrameCount > 0
          ? this.nativeWriteTimeMs / safeFrameCount
          : undefined,
    };
  }
  ...
  private disposeNativeH264Encoder(): void {
    ...
    try {
      ...
      console.debug(
        "[VideoExporter] Ignoring error closing native H.264 encoder:",
        error,
      );
    }
    ...
    throw new Error(
      `Video encoding not supported on this system. ` +
        `Tried encoder paths: ${encoderCandidates
          .map(
            (candidate) =>
              `${candidate.codec}/${candidate.hardwareAcceleration}`,
          )
  ...
  cancel(): void {
    ...
    if (nativeStaticLayoutSessionId && typeof window !== "undefined") {
      void window.electronAPI?.nativeStaticLayoutExportCancel?.(
        nativeStaticLayoutSessionId,
      );
    }
  }
  ...
  private cleanup(): void {
    ...
    this.lastFinalizationRenderProgress =
      INITIAL_FINALIZATION_PROGRESS_STATE.lastRenderProgress;
    this.lastFinalizationAudioProgress =
      INITIAL_FINALIZATION_PROGRESS_STATE.lastAudioProgress;
