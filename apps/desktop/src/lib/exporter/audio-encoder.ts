...
import { resolveSourceAudioFallbackPaths } from "./source-audio-fallback";
...
export class AudioProcessor {
      ...
      while (!this.cancelled) {
        ...
        const outputChunk =
          passthroughTimestampOffsetUs === 0
            ? chunk
            : this.cloneEncodedAudioChunkWithTimestamp(
                chunk,
                normalizedTimestamp,
              );
  ...
  async process(
    ...
    const sortedSpeedRegions = speedRegions
      ? [...speedRegions]
          .filter(
            (region) =>
              region.endMs - region.startMs > MIN_SPEED_REGION_DELTA_MS,
          )
    ...
    const sortedSourceAudioFallbackPaths = sourceAudioFallbackPaths
      ...
      : [];
    const { hasEmbeddedSourceAudio, externalAudioPaths } =
      resolveSourceAudioFallbackPaths(videoUrl, sortedSourceAudioFallbackPaths);
    const hasTimedCompanionAudio = externalAudioPaths.some(
      (audioPath) =>
        (sourceAudioFallbackStartDelayMsByPath?.[audioPath] ?? 0) > 0,
    );
  ...
  private async processTrimOnlyAudio(
    ...
    if (!codecCheck.supported) {
      console.warn(
        "[AudioProcessor] Audio codec not supported:",
        audioConfig.codec,
      );
      return;
    }
    ...
    await this.transcodeAudioStream(
      ...
      muxer,
      {
        ...
        shouldSkipChunk: (timestampMs) =>
          this.isInTrimRegion(timestampMs, sortedTrims),
        transformAudioData: (data) => {
          ...
          const adjustedTimestampUs =
            ...
            trimOffsetMs * 1000;
          return this.cloneWithTimestamp(
            data,
  ...
  private async transcodeAudioStream(
    ...
    if (!encodeSupport.supported) {
      console.warn(
        "[AudioProcessor] AAC encoding not supported, skipping audio",
      );
      return;
    }
    ...
    const encoder = new AudioEncoder({
      ...
      error: (error: DOMException) => {
        encodeError = new Error(
          `[AudioProcessor] Encode error: ${error.message}`,
        );
        notifyCapacityAvailable();
      },
    });
    ...
    const decoder = new AudioDecoder({
      ...
      error: (error: DOMException) => {
        decodeError = new Error(
          `[AudioProcessor] Decode error: ${error.message}`,
        );
        notifyCapacityAvailable();
      },
    });
    ...
    try {
      ...
      while (
        !this.cancelled &&
        (pendingFrames.length > 0 || encoder.encodeQueueSize > 0)
      ) {
        failIfNeeded();
        pumpEncodedFrames();
  ...
  private async renderAndEncodeChunked(
    ...
    if (!supported.supported) {
      console.warn(
        "[AudioProcessor] AAC encoding not supported for offline audio",
      );
      return;
    }
  ...
  private async bulkDecodeFromUrl(
    ...
    try {
      ...
      console.warn(
        "[AudioProcessor] Failed to decode audio from URL:",
        url,
        error,
      );
      return null;
  ...
  private buildTimelineSlices(
    ...
    for (const trim of trimRegions) {
      ...
      if (trim.endMs >= 0 && trim.endMs <= sourceDurationMs)
        boundaries.add(trim.endMs);
    }
    for (const speed of speedRegions) {
      if (speed.startMs >= 0 && speed.startMs <= sourceDurationMs)
        boundaries.add(speed.startMs);
      if (speed.endMs >= 0 && speed.endMs <= sourceDurationMs)
        boundaries.add(speed.endMs);
    }
  ...
  private scheduleBufferThroughTimeline(
    ...
    for (const slice of slices) {
      const sliceSourceDurationSec =
        (slice.sourceEndMs - slice.sourceStartMs) / 1000;
      ...
      let localOutputStartSec =
        outputOffsetSec +
        trimmedFromStartSec / slice.speed -
        chunkOutputStartSec;
      const localOutputEndSec =
      ...
      for (let i = 0; i < chunkFrames; i++) {
        for (let ch = 0; ch < numChannels; ch++) {
          const sample = Math.max(
            -1,
            Math.min(1, channels[ch][frameOffset + i]),
          );
          view.setInt16(
            byteOffset,
  ...
  private async loadAudioFileDemuxer(
    audioPath: string,
  ): Promise<WebDemuxer | null> {
    try {
      ...
      console.warn(
        "[AudioProcessor] Failed to create demuxer for sidecar audio:",
        error,
      );
      return null;
    }
  ...
  private isInTrimRegion(timestampMs: number, trims: TrimLikeRegion[]) {
    return trims.some(
      (trim) => timestampMs >= trim.startMs && timestampMs < trim.endMs,
    );
  }
