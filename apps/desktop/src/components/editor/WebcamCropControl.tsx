...
import { normalizeWebcamCropRegion } from "./utils/webcam-overlay";
...
export function WebcamCropControl({
  ...
  const [draftVisualCrop, setDraftVisualCrop] = useState<CropRegion | null>(
    null,
  );
  const [previewFrame, setPreviewFrame] = useState<PreviewFrame | null>(null);
  const hasPreviewFrame =
    previewSrc !== null && previewFrame?.src === previewSrc;
  ...
  const syncPreviewMedia = useCallback(() => {
    ...
    const webcamDuration = Number.isFinite(video.duration)
      ? video.duration
      : null;
  ...
  const handleKeyboardAdjust = (
    ...
    event.stopPropagation();
    commitVisualCrop(
      resizeCrop(crop, handle, delta.x, delta.y, previewAspectRatio),
      true,
    );
  };
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "absolute border border-white shadow-[0_0_0_1px_rgba(240,128,48,0.9),0_8px_24px_rgba(0,0,0,0.25)] focus:outline-none focus:ring-2 focus:ring-primary/60 focus:ring-offset-2 focus:ring-offset-editor-dialog",
