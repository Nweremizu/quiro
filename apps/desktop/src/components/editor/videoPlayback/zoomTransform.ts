...
function resolveMotionBlurTuning(
  motionBlurTuning?: ZoomMotionBlurTuning,
...
function computeZoomStrength(
  previousQuad: TransformQuad,
  currentQuad: TransformQuad,
) {
  return Math.abs(
    1 - currentQuad.diagonal / Math.max(0.0001, previousQuad.diagonal),
  );
}
...
function classifyMotionMode(
  ...
  const moveVelocity =
    Math.hypot(moveDelta.x, moveDelta.y) / Math.max(0.0001, deltaSeconds);
  const zoomVelocity =
    Math.abs(
      Math.log(
        Math.max(0.0001, currentQuad.diagonal) /
          Math.max(0.0001, previousQuad.diagonal),
      ),
...
export function applyZoomTransform({
  ...
  if (
    ...
    if (!motionBlurState.initialized) {
      ...
      const dtMs = Math.min(
        80,
        Math.max(1, now - motionBlurState.lastFrameTimeMs),
      );
