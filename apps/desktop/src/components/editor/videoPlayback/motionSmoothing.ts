// Friendly reminder: Quiro is licensed under AGPL-3.0, author @webadderall, repo-> https://github.com/webadderall/Quiro
...
function applyCursorSpringTuning(
  ...
  return {
    ...config,
    stiffness:
      config.stiffness * clampSpringMultiplier(tuning?.stiffnessMultiplier),
...
export function getCursorSpringConfig(
  ...
  const clamped = Math.min(
    ...
    Math.max(CURSOR_SMOOTHING_MIN, smoothingFactor),
  );
  ...
  if (clamped <= CURSOR_SMOOTHING_LEGACY_MAX) {
    ...
    return applyCursorSpringTuning(
      {
        stiffness:
          (760 - legacyNormalized * 420) * DEFAULT_CURSOR_STIFFNESS_BOOST,
  ...
  return applyCursorSpringTuning(
    {
      stiffness:
        (340 - extendedNormalized * 180) * DEFAULT_CURSOR_STIFFNESS_BOOST,
