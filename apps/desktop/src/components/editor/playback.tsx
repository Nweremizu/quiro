...
const VideoPlayback = forwardRef<VideoPlaybackRef, VideoPlaybackProps>(
  (
    ...
    const [pixiRendererError, setPixiRendererError] = useState<string | null>(
      null,
    );
    const [pixiRendererBackend, setPixiRendererBackend] =
      useState<PixiPreviewBackend | null>(null);
    ...
    const cursorSpringStiffnessMultiplierRef = useRef(
      cursorSpringStiffnessMultiplier,
    );
    const cursorSpringDampingMultiplierRef = useRef(
      cursorSpringDampingMultiplier,
    );
    const cursorSpringMassMultiplierRef = useRef(cursorSpringMassMultiplier);
    const cameraSpringStiffnessMultiplierRef = useRef(
      cameraSpringStiffnessMultiplier,
    );
    const cameraSpringDampingMultiplierRef = useRef(
    ...
    const syncPreviewMotionBlurQuality = useCallback(() => {
      ...
      const stageWidth = Math.max(
        1,
        stageSizeRef.current.width || app.screen.width,
      );
      const stageHeight = Math.max(
        1,
        stageSizeRef.current.height || app.screen.height,
      );

      motionBlurFilter.resolution = filterResolution;
      zoomBlurFilter.resolution = filterResolution;
      videoEffectsContainer.filterArea = new Rectangle(
        0,
        0,
        stageWidth,
        stageHeight,
    ...
    useEffect(() => {
      ...
      if (
        timelineJumped ||
        Math.abs(bgVideo.currentTime - targetTime) > driftThreshold
      ) {
        try {
    ...
    useEffect(() => {
      cursorSpringStiffnessMultiplierRef.current =
        cursorSpringStiffnessMultiplier;
    }, [cursorSpringStiffnessMultiplier]);
    ...
    useEffect(() => {
      cameraSpringStiffnessMultiplierRef.current =
        cameraSpringStiffnessMultiplier;
    }, [cameraSpringStiffnessMultiplier]);
    ...
    const syncWebcamMedia = useCallback(() => {
      ...
      const timelineJumped =
        previousTimelineTime === null ||
        Math.abs(targetTime - previousTimelineTime) > 0.25;
      const driftThreshold = isPlaying ? 0.35 : 0.01;
      if (
        timelineJumped ||
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        zoomBlurFilterRef.current = new ZoomBlurFilter({
          strength: 0,
          maxKernelSize: 13,
        });
    ...
    useEffect(() => {
      ...
      const videoEffectsContainer = videoEffectsContainerRef.current;
      const videoContainer = videoContainerRef.current;
      if (!app || !videoSprite || !videoEffectsContainer || !videoContainer)
        return;
      ...
      const ticker = () => {
        ...
        const zoomSpringConfig = getZoomSpringConfig(
          zoomSmoothnessRef.current,
          {
        ...
        const extensionCanvasWidth =
          effectsCanvas?.width || stageSizeRef.current.width;
        const extensionCanvasHeight =
          effectsCanvas?.height || stageSizeRef.current.height;
    ...
    useEffect(() => {
      ...
      const refreshSelectedCursorStyle = async () => {
        ...
        try {
          ...
          console.warn(
            "Failed to refresh contributed cursor styles in preview:",
            error,
          );
          return;
        }
        ...
        {hasRendererFallback && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/60 p-2 text-center">
            <div className="rounded-md bg-black/70 px-3 py-1.5 text-xs text-white">
              ...
              <br />
              Fallback to 2D native preview so you can continue working while
