import type { CaptionEditTarget } from "./caption-editing";
import type {
...
function flattenCaptionWords(cues: CaptionCue[]) {
  ...
  cues.forEach((cue, cueIndex) => {
    ...
    const shouldForceCueBreak =
      previousCue !== null &&
      cue.startMs - previousCue.endMs >= CAPTION_BLOCK_GAP_BREAK_MS;
...
function buildCaptionPages(options: {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(options.lines.length / options.maxRows),
  );
  let lineIndex = 0;
  ...
  for (let pageIndex = 0; pageIndex < totalPages; pageIndex += 1) {
    ...
    if (options.hasWordTimings) {
      ...
      const lastWord =
        options.words[pageLines[pageLines.length - 1].endWordIndex];
      pages.push({
        lines: pageLines,
  ...
  if (!options.hasWordTimings) {
    ...
    pages.forEach((page, pageIndex) => {
      const pageWordCount = page.lines.reduce(
        (count, line) => count + line.words.length,
        0,
      );
      const proportionalDuration = Math.round(
        (cueDuration * pageWordCount) / totalWords,
      );
      const nextElapsed =
        pageIndex === pages.length - 1
          ...
          : Math.min(
              options.cue.endMs,
              elapsedMs + Math.max(1, proportionalDuration),
            );
  ...
  for (let index = 0; index < pages.length - 1; index += 1) {
    pages[index].endMs = Math.max(
      pages[index].startMs + 1,
...
function buildCaptionLines(options: {
  ...
  const plainWordWidths = options.words.map((word) =>
    options.measureText(word.text),
  );
  const bestCost = new Array<number>(wordCount + 1).fill(
  ...
  for (let startIndex = wordCount - 1; startIndex >= 0; startIndex -= 1) {
    ...
    for (let endIndex = startIndex; endIndex < wordCount; endIndex += 1) {
      ...
      const fullness =
        options.maxWidthPx <= 0 ? 1 : lineWidth / options.maxWidthPx;
...
function getVisibleCaptionPageIndex(
...
export function buildActiveCaptionLayout(options: {
  ...
  if (activeWordIndex < 0) {
    activeWordIndex = sourceWords.findIndex(
      (word) => options.timeMs < word.startMs,
    );
    activeWordIndex =
      activeWordIndex < 0
  ...
  const animationEndMs =
    visiblePage?.endMs ?? sourceWords[sourceWords.length - 1].endMs;
  const pageStartWordIndex = visibleLines[0]?.startWordIndex ?? 0;
  const visibleWords = visibleLines.flatMap((line) => line.words);
  const visibleHasWordTimings = visibleWords.every(
    (word) => word.hasRealTiming,
  );
  const pageCueId =
    sourceWords[pageStartWordIndex]?.cueId ?? sourceWords[0].cueId;
  const activeCue =
  ...
  const enterProgress = clamp01(
    (options.timeMs - animationStartMs) / CAPTION_ENTER_MS,
  );
  const exitProgress = clamp01(
    (animationEndMs - options.timeMs) / CAPTION_EXIT_MS,
  );
