...
export const EDITOR_PREFERENCES_STORAGE_KEY = "quiro.editor.preferences";
export const EDITOR_PRESETS_STORAGE_KEY = "quiro.editor.presets";
...
export const DEFAULT_EDITOR_PREFERENCES: EditorPreferences = {
  ...
  zoomMotionBlurShutterFraction:
    DEFAULT_EDITOR_CONTROLS.zoomMotionBlurShutterFraction,
  ...
  cursorSpringStiffnessMultiplier:
    DEFAULT_EDITOR_CONTROLS.cursorSpringStiffnessMultiplier,
  cursorSpringDampingMultiplier:
    DEFAULT_EDITOR_CONTROLS.cursorSpringDampingMultiplier,
  cursorSpringMassMultiplier:
    DEFAULT_EDITOR_CONTROLS.cursorSpringMassMultiplier,
  cameraSpringStiffnessMultiplier:
    DEFAULT_EDITOR_CONTROLS.cameraSpringStiffnessMultiplier,
  cameraSpringDampingMultiplier:
    DEFAULT_EDITOR_CONTROLS.cameraSpringDampingMultiplier,
  cameraSpringMassMultiplier:
    DEFAULT_EDITOR_CONTROLS.cameraSpringMassMultiplier,
...
function normalizeCustomWallpapers(
  value: unknown,
  fallback: string[],
): string[] {
...
function normalizePresetAutoCaptionSettings(
  value: unknown,
): PresetAutoCaptionSettings {
...
function normalizeEditorPresetSnapshot(
  candidate: unknown,
  ...
  return {
    ...
    autoCaptionSettings: normalizePresetAutoCaptionSettings(
      raw.autoCaptionSettings,
    ),
    whisperExecutablePath:
      normalizeNullablePath(raw.whisperExecutablePath) ??
      normalizedPreferences.whisperExecutablePath,
    whisperModelPath:
      normalizeNullablePath(raw.whisperModelPath) ??
      normalizedPreferences.whisperModelPath,
  };
}
...
function normalizeEditorControls(
  ...
  const candidate: PartialEditorControls = {
    ...
    zoomTemporalMotionBlur:
      raw.zoomTemporalMotionBlur ?? fallback.zoomTemporalMotionBlur,
    zoomMotionBlurSampleCount:
      raw.zoomMotionBlurSampleCount ?? fallback.zoomMotionBlurSampleCount,
    zoomMotionBlurShutterFraction:
      raw.zoomMotionBlurShutterFraction ??
      fallback.zoomMotionBlurShutterFraction,
...
export function normalizeEditorPreferences(
  ...
  const raw =
    candidate && typeof candidate === "object"
      ? (candidate as Partial<EditorPreferences>)
      : {};
...
export function saveEditorPreferences(
  ...
  try {
    ...
    const merged = normalizeEditorPreferences(
      { ...current, ...preferences },
      current,
    );
