import type { CaptionCue, CaptionCueWord } from "@/types/editor";
  ...
  return tokens.map((token, index) => {
    ...
    const nextBoundaryMs =
      index === tokens.length - 1
        ? normalizedEndMs
        : Math.round(
            normalizedStartMs + (durationMs * (index + 1)) / tokens.length,
          );
    const wordEndMs = Math.min(
      normalizedEndMs,
      Math.max(wordStartMs + 1, nextBoundaryMs),
    );
  ...
  return sourceWords
    ...
    .map((word) => {
      ...
      const endMs = Math.max(
        startMs + 1,
        Math.min(cue.endMs, Math.round(word.endMs)),
      );
...
function normalizeCaptionWordSpacing(
...
export function updateCaptionCuesForEditedTarget(
  ...
  const tokenCountsByCue = distributeEditedTokensAcrossCueSegments(
    tokens.length,
    cueSegments,
  );
  let tokenCursor = 0;
  ...
  for (const segment of cueSegments) {
    ...
    editedWordsByCue.set(
      segment.cue.id,
      buildCaptionWordsForEditedText(
        segmentTokens.join(" "),
        segment.startMs,
        segment.endMs,
      ),
  ...
  return cues.map((cue) => {
    ...
    const keptWords = existingWords.filter(
...
function distributeEditedTokensAcrossCueSegments(
  ...
  if (tokenCount < segments.length) {
    ...
    const selectedCueIds = new Set(
      largestSegments.map((segment) => segment.cue.id),
    );
    for (const segment of segments) {
      tokenCountsByCue.set(
        segment.cue.id,
        selectedCueIds.has(segment.cue.id) ? 1 : 0,
      );
    }
    return tokenCountsByCue;
  }
  ...
  const weightedCounts = segments.map((segment) => {
    const exactCount =
      (Math.max(1, segment.endMs - segment.startMs) / totalDuration) *
      remainingTokens;
  ...
  let assignedTokens = weightedCounts.reduce(
    (total, item) => total + item.count,
    0,
  );

  for (const item of [...weightedCounts].sort(
