...
export default function TimelineWrapper({
    ...
    (candidate: Range): Range => {
      if (totalMs === 0) {
        ...
        const start = Math.max(
          0,
          Math.min(candidate.start, candidate.end - span),
        );
  ...
  const getSiblingSpans = useCallback(
    (activeItemId: string, rowId?: string) => {
      const activeItem = allRegionSpans.find(
        (region) => region.id === activeItemId,
      );
      ...
      return allRegionSpans
        .filter(
          (region) =>
            region.id !== activeItemId && region.rowId === resolvedRowId,
        )
    ...
    (span: Span, activeItemId: string): Span => {
      ...
      if (end - start < minDur) {
        const resizedLeft = Boolean(
          activeItem &&
          span.start !== activeItem.start &&
          span.end === activeItem.end,
        );
        if (resizedLeft) {
          start = end - minDur;
        } else {
  ...
  const clampDraggedSpanToNeighbours = useCallback(
    (span: Span, activeItemId: string, rowId?: string): Span => {
      ...
      const proposedStart = Number.isFinite(span.start)
        ? span.start
        : activeItem.start;
      ...
      const nextSibling = siblings.find(
        (region) => region.start >= activeItem.end,
      );
    ...
    (span: { start: number; end: number } | null, screenX?: number) => {
      ...
      if (screenX !== undefined) {
        ...
        if (parent) {
          ...
          const x = Math.max(
            0,
            Math.min(screenX - rect.left, rect.width - 100),
          );
          el.style.left = `${x}px`;
        }
      }
