...
import { type ComponentProps, type SVGProps, useEffect, useState } from "react";
import { useTitlebarState } from "@/utils/titlebar-state";
import { WindowButton as ControlButton } from "./window-button";
...
interface WindowControlsWindowsProps extends Omit<
  ComponentProps<"div">,
  "className"
> {
  className?: string;
...
export default function WindowControlsWindows({
  className,
  maximizable: maximizableProp,
  maximized: maximizedProp,
  onMaximize,
  ...props
}: WindowControlsWindowsProps) {
  const currentWindow = getCurrentWindow();
  // NOTE: assumes titlebarState has been ported to a React hook that
  // re-renders this component on change (e.g. a zustand/valtio store).
  // Swap this for however that module actually exposes state in React.
  const titlebarState = useTitlebarState();
  const [focused, setFocused] = useState(true);

  const hasCustomMaximize = onMaximize !== undefined;
  const maximizable =
    maximizableProp !== false &&
    (hasCustomMaximize || titlebarState.maximizable);
  const maximized = maximizedProp ?? titlebarState.maximized;
  const showMaximizeButton =
    maximizable || !titlebarState.hideMaximize || maximizableProp;
  ...
  const handleMaximize = maximizable
    ? (onMaximize ??
      (() => {
        void (maximized ? currentWindow.unmaximize() : currentWindow.maximize());
      }))
    : undefined;
  ...
  return (
    ...
    >
      <ControlButton
        ...
        onClick={titlebarState.minimizable ? handleMinimize : undefined}
      ...
      <ControlButton
        ...
        className={cn(
          ...
          "hover:text-gray-1 hover:bg-[#c42b1c] dark:hover:bg-[#c42b1c] active:bg-[#c42b1c]/90 dark:active:bg-[#c42b1c]/90",
        )}
      >
        <icons.closeWin />
      </ControlButton>
