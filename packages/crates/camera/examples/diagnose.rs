...
use quiro_camera::{CameraInfo, CaptureMode, Format};
...
fn main() {
    ...
    let cameras: Vec<CameraInfo> = quiro_camera::list_cameras().collect();
    if cameras.is_empty() {
...
fn describe_frame(frame: &quiro_camera::CapturedFrame) -> String {
    #[cfg(target_os = "macos")]
    {
