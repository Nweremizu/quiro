...
fn init_tracing() {
    let filter = std::env::var("QUIRO_MUXER_LOG").unwrap_or_else(|_| "info".to_string());
...
fn resolve_queue_capacity() -> usize {
    std::env::var("QUIRO_MUXER_IO_BUF_MB")
        .ok()
...
fn run() -> Result<(), MuxerError> {
    ...
    tracing::info!(
        ...
        "quiro-muxer async frame queue initialized"
    );

    let reader_handle = {
        let queue = Arc::clone(&queue);
        std::thread::Builder::new()
            .name("quiro-muxer-stdin-reader".to_string())
            .spawn(move || {
                let stdin = io::stdin();
    ...
    loop {
        match queue.pop() {
            ...
            PopResult::Drained => {
                tracing::warn!(
                    "quiro-muxer stdin closed without Finish; attempting graceful finish"
                );
                break;
            }
            PopResult::Error(e) => {
    ...
    writeln!(io::stderr(), "quiro-muxer finished cleanly").ok();
    Ok(())
}
...
impl State {
    fn finish(&mut self) -> Result<(), MuxerError> {
        ...
        if let Some(mut out) = self.output.take() {
            ...
            writeln!(
                io::stderr(),
                "quiro-muxer trailer written; video_packets={} audio_packets={} duration_secs={:.2}",
    ...
    if packet.stream_index == STREAM_INDEX_VIDEO {
        ...
        if !out.video_keyframe_seen {
            if !is_keyframe {
                    ...
                    || out.video_packets_dropped_pre_keyframe.is_multiple_of(30)
                {
                    tracing::warn!(
                        ...
                        "quiro-muxer discarding non-keyframe video packets before first keyframe"
                    );
                }
                return Ok(());
            }
            ...
            if out.video_packets_dropped_pre_keyframe > 0 {
                tracing::info!(
                    dropped = out.video_packets_dropped_pre_keyframe,
                    "quiro-muxer first video keyframe received"
                );
            }
        }
        ...
        && !out.video_keyframe_seen
    {
            ...
            || out.audio_packets_dropped_pre_video.is_multiple_of(100)
        {
            tracing::debug!(
                ...
                "quiro-muxer holding audio until first video keyframe"
            );
        }
        return Ok(());
    }
