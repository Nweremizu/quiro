...
use quiro_editor::{AudioRenderer, get_audio_segments, load_music_tracks_uncached};
use quiro_enc_ffmpeg::{AudioEncoder, aac::AACEncoder, h264::H264Encoder, mp4::*};
use quiro_media_info::{RawVideoFormat, VideoInfo};
...
struct ExportNv12Mode {
    ...
    nv12_render_startup_breakdown_ms:
        Option<Arc<Mutex<Option<quiro_rendering::Nv12RenderStartupBreakdownMs>>>>,
}

#[derive(Debug, serde::Serialize)]
pub struct FirstFrameQueuedBenchmark {
    pub ms_to_first_frame_queued_since_export_pipeline_start: u64,
    pub nv12_render_startup_breakdown_ms: Option<quiro_rendering::Nv12RenderStartupBreakdownMs>,
}
...
fn save_screenshot_from_nv12(
    ...
    quiro_rendering::cpu_yuv::nv12_to_rgba_simd(
...
use quiro_project::{ProjectConfiguration, RecordingMeta, StudioRecordingMeta};
use quiro_rendering::{ProjectRecordingsMeta, RenderVideoConstants};
...
async fn export_render_to_channel(
    ...
    stop_after_frames_sent: Option<u32>,
    startup_breakdown_ms: Option<Arc<Mutex<Option<quiro_rendering::Nv12RenderStartupBreakdownMs>>>>,
    mut on_progress: impl FnMut(u32) -> bool + Send + 'static,
    project_path: PathBuf,
) -> Result<(), quiro_rendering::RenderingError> {
    let (tx_image_data, mut video_rx) = tokio::sync::mpsc::channel::<(Nv12RenderedFrame, u32)>(8);

    let screenshot_project_path = project_path;

    let render_result = {
        let render_future = quiro_rendering::render_video_to_channel_nv12(
            constants,
            project,
            tx_image_data,
            recording_meta,
        ...
        let forward_future = async {
            ...
            loop {
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        if consecutive_timeouts >= MAX_CONSECUTIVE_FRAME_TIMEOUTS {
                            return Err(quiro_rendering::RenderingError::ImageLoadError(format!(
                ...
                if !(on_progress)(frame_count) {
                    return Err(quiro_rendering::RenderingError::ImageLoadError(
            ...
            Ok::<_, quiro_rendering::RenderingError>(())
        };
...
mod tests {
    ...
    fn nv12_from_rendered_frame_passthrough_for_nv12_format() {
        use quiro_rendering::{GpuOutputFormat, Nv12RenderedFrame};
