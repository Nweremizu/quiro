...
use quiro_project::{BackgroundConfiguration, BackgroundSource};
...
/// Film-grain noise applied over any background source. Resolved from
/// `BackgroundConfiguration::noise_*` (falling back to a gradient source's own
/// noise) and threaded into both background pipelines. `intensity == 0` renders
/// identically to no noise.
...
impl BackgroundNoise {
    /// The explicit background-level setting if present, otherwise a gradient
    /// source's own noise (so projects saved before the background-level field
    /// existed still grain), otherwise none.
    pub fn resolve(background: &BackgroundConfiguration) -> Self {
        let (gradient_intensity, gradient_scale) = match &background.source {
            BackgroundSource::Gradient {
                noise_intensity,
                noise_scale,
                ..
            } => (*noise_intensity, *noise_scale),
            _ => (None, None),
        };
        Self {
            intensity: background
                .noise_intensity
                .or(gradient_intensity)
                .unwrap_or(0.0),
            scale: background.noise_scale.or(gradient_scale).unwrap_or(3.0),
        }
    }
}
        ...
        match background {
            Background::Image { path } => {
                match &self.inner {
                    Some(Inner::Image {
                        ...
                        noise: current_noise,
                        ..
                    }) if current_path == &path && *current_noise == noise => {}
                    _ => {
            ...
            Background::Color(color) => match &self.inner {
                Some(Inner::ColorOrGradient {
                    ...
                    noise: current_noise,
                    ..
                }) if &color == current_color && *current_noise == noise => {}
                _ => {
                    let buffer = GradientOrColorUniforms::from(background)
                        .with_noise(noise)
                        .to_buffer(device);
                    self.inner = Some(Inner::ColorOrGradient {
            ...
            Background::Gradient(gradient) => match &self.inner {
                Some(Inner::ColorOrGradient {
                    ...
                    noise: current_noise,
                    ..
                }) if &gradient == current_gradient && *current_noise == noise => {}
                _ => {
                    let buffer = GradientOrColorUniforms::from(background)
                        .with_noise(noise)
                        .to_buffer(device);
...
pub struct ImageBackgroundUniforms {
    ...
    noise_intensity: f32,
    noise_scale: f32,
    _padding: f32, // Pads the struct to 32 bytes to match the WGSL layout.
}
...
impl GradientOrColorUniforms {
    /// Overrides the noise channels with the caller-resolved background noise.
    /// The `From<Background>` impl fills these from a gradient source (or zero
    /// for a solid colour); this lets the background-level setting win.
    fn with_noise(mut self, noise: BackgroundNoise) -> Self {
        self.noise_intensity = noise.intensity;
...
mod tests {
    use super::*;
    use quiro_project::BackgroundSource;
