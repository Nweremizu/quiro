...
// Sends a screen pixel back to the card plane. The card is flat, so the whole
// projection is one homography and there is no geometry to transform — which
// suits this shader, since it draws the card from a fullscreen triangle with an
// SDF rather than a quad.
//
// `w` comes back with it: negative means the pixel is behind the camera, where
// the perspective divide would fold the image back on itself.
struct CardPoint {
    xy: vec2<f32>,
    w: f32,
};

fn to_card_space(p: vec2<f32>) -> CardPoint {
    let v = uniforms.inv_perspective * vec3<f32>(p, 1.0);
    var out: CardPoint;
    out.w = v.z;
    // Guarded so a degenerate matrix cannot produce a NaN that poisons the SDF.
    let safe_w = select(1.0, v.z, abs(v.z) > 1e-6);
    out.xy = v.xy / safe_w;
    return out;
}
...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    // exactly frag_coord.xy, so the SDF, border, UV and motion-blur paths are
    // untouched for every caller that does not opt in.
    let card = to_card_space(frag_coord.xy);
    if card.w <= 0.0 {
        return vec4<f32>(0.0);
    }
    let p = card.xy;
