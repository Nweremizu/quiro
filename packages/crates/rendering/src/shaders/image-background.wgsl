struct Uniforms {
    ...
    noise_intensity: f32,
    noise_scale: f32,
    _padding: f32,  // Pads the struct to 32 bytes to match the Rust layout.
};
...
// Film-grain noise, identical to gradient-or-color.wgsl so a wallpaper/image
// background grains the same way a colour or gradient one does.
