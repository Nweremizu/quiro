...
use quiro_video_decode::FrameTextures;
...
impl FfmpegDecoder {
    ...
    pub fn spawn_with_hw_config(
        ...
        std::thread::spawn(move || {
            ...
            let mut this =
                match quiro_video_decode::FFmpegDecoder::new(path.clone(), hw_device_type) {
                    Err(e) => {
            ...
            if first_frame_result.is_none() && is_hw {
                let mut sw_this = match quiro_video_decode::FFmpegDecoder::new(path.clone(), None) {
                    Err(e) => {
