...
use quiro_media_info::{Pixel, VideoInfo, ensure_even};
...
fn get_default_encoder_priority(_config: &VideoInfo) -> &'static [&'static str] {
    ...
    // ponytail: GPU-vendor priority list (cap-frame-converter) not copied
    // over yet — software libx264 only. requires_software_encoder() above
    // already forces this path on Windows, so this branch is unreachable
    // today; kept simple rather than deleted so re-adding hardware encoders
    // later is a priority-list change, not new plumbing.
    #[cfg(target_os = "windows")]
    {
        let _ = _config;
        &["libx264"]
...
fn export_encoder_priority_override(
    ...
    // ponytail: AMD export override (cap-frame-converter) not copied over yet.
    None
}
...
mod self_test_tests {
    ...
    fn self_test_round_trip_passes_on_software_encoder() {
        ...
        let config = VideoInfo::from_raw(quiro_media_info::RawVideoFormat::Bgra, 160, 120, 30);
