...
fn test_swscale_conversion_works() {
    ...
    let config = quiro_frame_converter::ConversionConfig::new(
    ...
    let result = quiro_frame_converter::create_converter_with_details(config);
    assert!(
        result.is_ok(),
...
fn test_system_diagnostics_collection() {
    ...
    let diagnostics = quiro_recording::diagnostics::collect_diagnostics();
...
fn test_gpu_detection() {
    ...
    let gpu_info = quiro_frame_converter::detect_primary_gpu();
    ...
    if let Some(info) = gpu_info {
        ...
        match info.vendor {
            quiro_frame_converter::GpuVendor::Nvidia => {
                println!("  -> NVIDIA GPU: NVENC encoding expected");
            }
            quiro_frame_converter::GpuVendor::Amd => {
                println!("  -> AMD GPU: AMF encoding expected");
            }
            quiro_frame_converter::GpuVendor::Intel => {
                println!("  -> Intel GPU: QSV encoding expected");
            }
            quiro_frame_converter::GpuVendor::Qualcomm => {
                println!("  -> Qualcomm GPU: Software encoding expected");
            }
            quiro_frame_converter::GpuVendor::Arm => {
                println!("  -> ARM GPU: Software encoding expected");
            }
            quiro_frame_converter::GpuVendor::Microsoft => {
                println!("  -> Microsoft WARP: Software rendering/encoding");
            }
            quiro_frame_converter::GpuVendor::Unknown(id) => {
...
fn test_camera_enumeration() {
    ...
    let cameras: Vec<quiro_camera::CameraInfo> = quiro_camera::list_cameras().collect();
...
fn test_d3d11_converter_capability() {
    ...
    for (name, input, output, width, height) in test_configs {
        ...
        match quiro_frame_converter::create_converter_with_details(config) {
            Ok(result) => {
...
fn test_supported_pixel_formats() {
    ...
    for (format, name) in formats {
        let supported = quiro_frame_converter::is_format_supported(format);
...
fn test_frame_conversion_performance() {
    ...
    let result = quiro_frame_converter::create_converter_with_details(config.clone());
    if result.is_err() {
...
fn test_multi_gpu_detection() {
    ...
    if let Some(gpu) = quiro_frame_converter::detect_primary_gpu() {
...
fn test_minimum_requirements_check() {
    ...
    println!("\nRecommended:");
    if quiro_frame_converter::detect_primary_gpu().is_some() {
