...
async fn instant_record_with_real_mic_and_screen() {
    ...
    let mic_feed = if let Some((label, _device, _config)) = &default_mic {
        ...
        let ready_future = mic_actor
            .ask(quiro_recording::feeds::microphone::SetInput {
                settings: None,
                label: label.clone(),
            })
            .await
        ...
        match mic_actor
            .ask(quiro_recording::feeds::microphone::Lock)
            .await
        {
            Ok(lock) => {
    ...
    if let Some(rx) = segment_rx {
        let events: Vec<quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent> =
            rx.try_iter().collect();
        ...
        let video_events = events
            .iter()
            .filter(|e| {
                e.media_type == quiro_enc_ffmpeg::segmented_stream::SegmentMediaType::Video
                    && !e.is_init
            })
            .count();
        let audio_events = events
            .iter()
            .filter(|e| {
                e.media_type == quiro_enc_ffmpeg::segmented_stream::SegmentMediaType::Audio
                    && !e.is_init
            })
            .count();
    ...
    match &completed.health {
        quiro_recording::RecordingHealth::Healthy => {
            eprintln!("\nRecording health: HEALTHY");
        }
        quiro_recording::RecordingHealth::Repaired { original_issue } => {
            eprintln!("\nRecording health: REPAIRED (was: {original_issue})");
        }
        quiro_recording::RecordingHealth::Degraded { issues } => {
            eprintln!("\nRecording health: DEGRADED - {issues:?}");
        }
        quiro_recording::RecordingHealth::Damaged { reason } => {
