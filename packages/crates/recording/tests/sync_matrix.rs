...
use quiro_media_info::{AudioInfo, Sample, Type, VideoInfo};
use quiro_recording::{
...
async fn run_video_case(case: VideoCase) -> Result<String, String> {
    ...
    let info = VideoInfo::from_raw(
        quiro_media_info::RawVideoFormat::Bgra,
        case.width,
        case.height,
        case.fps,
    );
...
async fn run_video_pause_case() -> Result<String, String> {
    ...
    let info = VideoInfo::from_raw(quiro_media_info::RawVideoFormat::Bgra, 160, 120, FPS);
    let (tx, rx) = flume::bounded::<FFmpegVideoFrame>(32);
    let timestamps = Timestamps::now();
...
async fn run_system_audio_case(case: SystemAudioCase) -> Result<String, String> {
    use quiro_recording::AudioAnchor;
...
async fn warm_up_video_encoder() {
    ...
    let info = VideoInfo::from_raw(quiro_media_info::RawVideoFormat::Bgra, 160, 120, 30);
