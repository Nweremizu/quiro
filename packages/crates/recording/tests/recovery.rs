...
use quiro_recording::recovery::{RecoveryError, RecoveryManager};
...
fn synthetic_video_info() -> quiro_media_info::VideoInfo {
    quiro_media_info::VideoInfo {
        pixel_format: quiro_media_info::Pixel::NV12,
        width: 320,
        height: 240,
...
fn recover_after_simulated_crash_produces_playable_mp4_with_preserved_duration() {
    ...
    let duration = quiro_enc_ffmpeg::remux::get_media_duration(&display_mp4)
...
fn finalize_to_progressive_mp4_includes_respawn_fragments() {
    ...
    let duration = quiro_enc_ffmpeg::remux::get_media_duration(&output).expect("read duration");
    ...
    let baseline_duration =
        quiro_enc_ffmpeg::remux::get_media_duration(&single_dir_output).expect("baseline duration");
