...
use quiro_timestamp::{Timestamp, Timestamps};
...
fn to_project_gap_summary(
    ...
    summary.map(|s| quiro_project::AudioGapSummary {
...
pub struct CompletedRecording {
    ...
    pub cursor_data: quiro_project::CursorImages,
}
...
async fn stop_recording(
    ...
    use quiro_project::*;
    use quiro_timestamp::{AUDIO_OUTPUT_FRAMES, DEFAULT_SAMPLE_RATE};
    ...
    let meta = StudioRecordingMeta::MultipleSegments {
        inner: MultipleSegments {
            ...
            cursors: quiro_project::Cursors::Correct(
                cursors
                    .into_values()
                    .map(|cursor| {
                        (
    ...
    let mut project_config = quiro_project::ProjectConfiguration::default();
    ...
    let cursor = if camera_only {
        ...
        (custom_cursor_capture || keyboard_capture)
            .then(move || {
                ...
                let keyboard_output_path = dir.join(quiro_project::KEYBOARD_EVENTS_FILE_NAME);
...
fn write_in_progress_meta(recording_dir: &Path) -> anyhow::Result<()> {
    ...
    let meta = RecordingMeta {
        ...
        inner: RecordingMetaInner::Studio(Box::new(StudioRecordingMeta::MultipleSegments {
            inner: MultipleSegments {
                segments: Vec::new(),
                cursors: quiro_project::Cursors::default(),
...
mod tests {
    ...
    impl Muxer for SuccessfulVideoMuxer {
        ...
        async fn setup(
            ...
            _audio_config: Option<quiro_media_info::AudioInfo>,
    ...
    fn test_audio_info() -> quiro_media_info::AudioInfo {
        quiro_media_info::AudioInfo::new_raw(
