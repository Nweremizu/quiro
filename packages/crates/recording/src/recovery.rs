...
use quiro_enc_ffmpeg::fragmented_mp4::tail_is_complete;
...
macro_rules! finalization_info {
    ($($arg:tt)*) => {
        tracing::info!(target: "quiro_recording::recording_finalization", $($arg)*)
    };
}
...
pub enum RecoveryError {
    ...
    VideoConcat(quiro_enc_ffmpeg::remux::RemuxError),
    #[error("Failed to concatenate audio fragments: {0}")]
    AudioConcat(quiro_enc_ffmpeg::remux::RemuxError),
    #[error("Failed to merge media streams: {0}")]
    MediaMerge(quiro_enc_ffmpeg::remux::RemuxError),
    ...
    pub fn finalize_to_progressive_mp4_with_health(
        ...
        let concat_result = if group_outputs.len() == 1 {
            std::fs::rename(&group_outputs[0], output)
                .map_err(|e| RecoveryError::VideoConcat(quiro_enc_ffmpeg::remux::RemuxError::Io(e)))
        } else {
    ...
    fn build_recovered_meta(
        ...
        let segments: Vec<MultipleSegment> = recording
            ...
            .map(|seg| {
                ...
                let keyboard_path = {
                    let binary = segment_dir.join(quiro_project::KEYBOARD_EVENTS_FILE_NAME);
                    if binary.exists() {
                        binary
                    } else {
                        segment_dir.join(quiro_project::LEGACY_KEYBOARD_EVENTS_FILE_NAME)
                    }
                };
    ...
    fn scan_cursor_images(cursors_dir: &Path) -> Cursors {
        ...
        for entry in entries.flatten() {
                ...
                && let Some(full_file_name) = path.file_name().and_then(|n| n.to_str())
            {
                ...
                cursors.insert(
                    id_str.to_string(),
                    quiro_project::CursorMeta {
                        image_path: relative_path,
                        hotspot: quiro_project::XY::new(0.0, 0.0),
                        shape: None,
                    },
                );
