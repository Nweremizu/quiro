...
use quiro_project::InstantRecordingMeta;
use quiro_timestamp::Timestamps;
use quiro_utils::ensure_dir;
use std::{
    path::PathBuf,
    sync::Arc,
...
struct Pipeline {
    ...
    segment_rx: Option<
        std::sync::mpsc::Receiver<quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent>,
    >,
}
        ...
        std::sync::Mutex<
            Option<
                std::sync::mpsc::Receiver<
                    quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent,
                >,
            >,
        >,
    >,
...
impl ActorHandle {
    ...
    pub fn take_segment_rx(
        &self,
    ) -> Option<std::sync::mpsc::Receiver<quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent>>
    {
        self.segment_rx
            .as_ref()
...
async fn create_pipeline(
    ...
    screen_info: quiro_media_info::VideoInfo,
    ...
    let segment_channel = {
        let (tx, rx) =
            std::sync::mpsc::channel::<quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent>();
        Some((tx, rx))
    };
