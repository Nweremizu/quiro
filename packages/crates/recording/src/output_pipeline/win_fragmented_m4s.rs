...
use quiro_media_info::{AudioInfo, Pixel, VideoInfo};
use std::{
    path::PathBuf,
    sync::{
        Arc, Mutex,
...
pub struct WindowsFragmentedM4SMuxer {
    ...
    segment_tx:
        Option<std::sync::mpsc::Sender<quiro_enc_ffmpeg::segmented_stream::SegmentCompletedEvent>>,
    health_tx: SharedHealthSender,
}
...
impl WindowsFragmentedM4SCameraMuxer {
    fn normalize_video_config_for_frame(&mut self, frame: &NativeCameraFrame) {
        ...
        let desired_pixel_format = match frame.pixel_format {
            quiro_camera_windows::PixelFormat::UYVY422 => Pixel::YUYV422,
