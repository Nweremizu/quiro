...
use quiro_enc_ffmpeg::aac::AACEncoder;
use quiro_media_info::{AudioInfo, VideoInfo};
use quiro_timestamp::Timestamp;
use std::{
    path::PathBuf,
    sync::{
        Arc, Mutex,
...
impl Muxer for WindowsMuxer {
        ...
        Self: Sized,
    {
        ...
        if fragmented {
            quiro_mediafoundation_ffmpeg::set_fragmented_mp4_options(
                &mut output,
                frag_duration_us,
            )?;
        }
            ...
            tasks.spawn_thread("windows-encoder", move || {
                ...
                let encoder = (|| {
                    let fallback = |reason: Option<String>| {
                        ...
                        quiro_enc_ffmpeg::h264::H264Encoder::builder(video_config)
                    ...
                    match quiro_enc_mediafoundation::H264Encoder::new_with_scaled_output(
                        &config.d3d_device,
                        config.pixel_format,
                        input_size,
                        output_size,
                        ...
                        Ok(encoder) => {
                            ...
                            let muxer = {
                                ...
                                quiro_mediafoundation_ffmpeg::H264StreamMuxer::new(
                                    &mut output_guard,
                                    quiro_mediafoundation_ffmpeg::MuxerConfig {
                                        width,
                                        height,
...
pub struct NativeCameraFrame {
    ...
    pub pixel_format: quiro_camera_windows::PixelFormat,
    pub width: u32,
    pub height: u32,
...
impl NativeCameraFrame {
    pub fn dxgi_format(&self) -> DXGI_FORMAT {
        match self.pixel_format {
            quiro_camera_windows::PixelFormat::NV12 => DXGI_FORMAT_NV12,
            quiro_camera_windows::PixelFormat::YUYV422
            | quiro_camera_windows::PixelFormat::UYVY422 => DXGI_FORMAT_YUY2,
            quiro_camera_windows::PixelFormat::ARGB | quiro_camera_windows::PixelFormat::RGB32 => {
                DXGI_FORMAT_B8G8R8A8_UNORM
            }
            quiro_camera_windows::PixelFormat::RGB24 => DXGI_FORMAT_R8G8B8A8_UNORM,
            _ => DXGI_FORMAT_NV12,
        }
    }
}
...
fn infer_camera_buffer_layout(
    ...
    use quiro_camera_windows::PixelFormat;
    ...
    match pixel_format {
        PixelFormat::NV12 | PixelFormat::NV21 => {
            if primary_stride < width {
                return None;
            }
            Some(CameraBufferLayout {
                primary_stride,
                secondary_stride: primary_stride,
            })
        }
...
fn compact_primary_stride(pixel_format: quiro_camera_windows::PixelFormat, width: usize) -> usize {
    use quiro_camera_windows::PixelFormat;
    ...
    match pixel_format {
        ...
        PixelFormat::ARGB | PixelFormat::RGB32 => width * 4,
...
fn convert_uyvy_row_to_yuyv_into(src: &[u8], dst: &mut [u8]) {
    let src_len = src.len().min(dst.len());
...
fn convert_uyvy_to_yuyv_into(
    ...
    for row in 0..height {
        ...
        let src_start = src_row.saturating_mul(src_stride);
        let dst_start = row.saturating_mul(row_bytes);
        let src_end = src_start.saturating_add(row_bytes).min(src.len());
        let dst_end = dst_start.saturating_add(row_bytes).min(dst.len());

        if src_end <= src_start || dst_end <= dst_start || dst_end - dst_start < row_bytes {
            continue;
        }

        convert_uyvy_row_to_yuyv_into(&src[src_start..src_end], &mut dst[dst_start..dst_end]);
    }
}
...
fn convert_uyvy_to_yuyv(
    ...
    convert_uyvy_to_yuyv_into(src, &mut dst, width, height, src_stride, flip);
    dst
}

pub fn camera_frame_to_ffmpeg(frame: &NativeCameraFrame) -> anyhow::Result<ffmpeg::frame::Video> {
    use quiro_mediafoundation_utils::IMFMediaBufferExt;

    if frame.pixel_format == quiro_camera_windows::PixelFormat::MJPEG {
        return decode_mjpeg_frame(frame);
    }

    let ffmpeg_format = match frame.pixel_format {
        quiro_camera_windows::PixelFormat::NV12 => ffmpeg::format::Pixel::NV12,
        quiro_camera_windows::PixelFormat::YUYV422 => ffmpeg::format::Pixel::YUYV422,
        quiro_camera_windows::PixelFormat::UYVY422 => ffmpeg::format::Pixel::UYVY422,
        quiro_camera_windows::PixelFormat::ARGB | quiro_camera_windows::PixelFormat::RGB32 => {
            ffmpeg::format::Pixel::BGRA
        }
        quiro_camera_windows::PixelFormat::RGB24 => ffmpeg::format::Pixel::BGR24,
        quiro_camera_windows::PixelFormat::BGR24 => ffmpeg::format::Pixel::BGR24,
        quiro_camera_windows::PixelFormat::YUV420P => ffmpeg::format::Pixel::YUV420P,
        quiro_camera_windows::PixelFormat::YV12 => ffmpeg::format::Pixel::YUV420P,
        quiro_camera_windows::PixelFormat::NV21 => ffmpeg::format::Pixel::NV12,
    ...
    let layout = infer_camera_buffer_layout(frame.pixel_format, width, height, data.len())
        .ok_or_else(|| {
            anyhow!(
    ...
    let (final_data, final_format): (&[u8], ffmpeg::format::Pixel) =
        if frame.pixel_format == quiro_camera_windows::PixelFormat::UYVY422 {
    ...
    let flip =
        frame.is_bottom_up && frame.pixel_format != quiro_camera_windows::PixelFormat::UYVY422;

    match frame.pixel_format {
        quiro_camera_windows::PixelFormat::NV12 => {
            ...
            if final_data.len() >= y_size + uv_size {
                ...
                copy_plane(
                    ...
                    width,
                    uv_height,
                    layout.secondary_stride,
                    stride_uv,
                    flip,
                );
            }
        }
        quiro_camera_windows::PixelFormat::NV21 => {
            let uv_height = height / 2;
            let y_size = layout.primary_stride * height;
            let uv_size = layout.secondary_stride * uv_height;
            if final_data.len() >= y_size + uv_size {
                ...
                for row in 0..uv_height {
                    ...
                    for x in 0..width / 2 {
                        let src_idx = src_row * layout.secondary_stride + x * 2;
        ...
        quiro_camera_windows::PixelFormat::YUYV422 | quiro_camera_windows::PixelFormat::UYVY422 => {
            let row_bytes = width * 2;
            let size = if frame.pixel_format == quiro_camera_windows::PixelFormat::UYVY422 {
                row_bytes * height
            } else {
        ...
        quiro_camera_windows::PixelFormat::RGB24 | quiro_camera_windows::PixelFormat::BGR24 => {
        ...
        quiro_camera_windows::PixelFormat::YUV420P => {
            let uv_height = height / 2;
            let y_size = layout.primary_stride * height;
            let uv_size = layout.secondary_stride * uv_height;
            if final_data.len() >= y_size + uv_size * 2 {
                ...
                copy_plane(
                    &final_data[y_size + uv_size..y_size + uv_size * 2],
        ...
        quiro_camera_windows::PixelFormat::YV12 => {
            let uv_height = height / 2;
            let y_size = layout.primary_stride * height;
            let uv_size = layout.secondary_stride * uv_height;
            if final_data.len() >= y_size + uv_size * 2 {
...
fn flip_camera_buffer_into(
    ...
    match pixel_format {
        PixelFormat::NV12 | PixelFormat::NV21 => {
            let y_size = layout.primary_stride * height;
            copy_plane(data, dst, width, height, layout.primary_stride, width, true);

            let uv_height = height / 2;
            let uv_row_size = width;
            for row in 0..uv_height {
                let src_row = uv_height - 1 - row;
                let src_start = y_size + src_row * layout.secondary_stride;
                let dst_start = width * height + row * uv_row_size;
        ...
        PixelFormat::YUV420P | PixelFormat::YV12 => {
            ...
            let uv_height = height / 2;
            let uv_plane_size = layout.secondary_stride * uv_height;
            ...
            for row in 0..uv_height {
                let src_row = uv_height - 1 - row;
                let src_u_start = u_offset + src_row * layout.secondary_stride;
                let dst_u_start = width * height + row * uv_width;
                ...
                let src_v_start = v_offset + src_row * layout.secondary_stride;
                let dst_v_start = width * height + (uv_width * uv_height) + row * uv_width;
        ...
        PixelFormat::P010 => {
            ...
            for row in 0..uv_height {
                let src_row = uv_height - 1 - row;
                let src_start = y_size + src_row * layout.secondary_stride;
                let dst_start = y_row_size * height + row * uv_row_size;
                if src_start + uv_row_size <= data.len() && dst_start + uv_row_size <= dst.len() {
                    dst[dst_start..dst_start + uv_row_size]
                        .copy_from_slice(&data[src_start..src_start + uv_row_size]);
                }
...
pub fn upload_mf_buffer_to_texture(
    ...
    let needs_uyvy_conversion = frame.pixel_format == quiro_camera_windows::PixelFormat::UYVY422;
    ...
    let data: &[u8] = match (needs_uyvy_conversion, needs_flip) {
        ...
        (false, true) => {
            ...
            row_pitch = compact_primary_stride(frame.pixel_format, frame.width as usize) as u32;
            dst
        }
        (true, true) => {
            let uyvy_size = (frame.width * frame.height * 2) as usize;
