...
mod windows_impl {
    ...
    fn gpu_info_to_diag(info: &quiro_frame_converter::GpuInfo) -> GpuInfoDiag {
        GpuInfoDiag {
    ...
    fn get_gpu_info() -> Option<GpuInfoDiag> {
        quiro_frame_converter::detect_primary_gpu().map(gpu_info_to_diag)
    }

    fn get_all_gpus_info() -> Option<AllGpusInfo> {
        let all_gpus = quiro_frame_converter::get_all_gpus();
        ...
        let primary_gpu = quiro_frame_converter::detect_primary_gpu();
    ...
    fn check_d3d11_video_processor() -> bool {
        use quiro_frame_converter::ConversionConfig;
        ...
        match quiro_frame_converter::D3D11Converter::new(test_config) {
            Ok(converter) => {
                tracing::debug!(
