...
use quiro_camera_ffmpeg::*;
use quiro_fail::fail_err;
...
struct AttachedState {
    #[allow(dead_code)]
    id: DeviceOrModelID,
    camera_info: quiro_camera::CameraInfo,
...
impl CameraFeedLock {
    pub fn camera_info(&self) -> &quiro_camera::CameraInfo {
        &self.camera_info
    }
...
pub enum DeviceOrModelID {
    DeviceID(String),
    ModelID(quiro_camera::ModelID),
}

impl DeviceOrModelID {
    pub fn from_info(info: &quiro_camera::CameraInfo) -> Self {
        info.model_id()
...
#[cfg(any(target_os = "macos", windows, target_os = "linux"))]
fn find_camera(selected_camera: &DeviceOrModelID) -> Option<quiro_camera::CameraInfo> {
    quiro_camera::list_cameras().find(|c| match selected_camera {
...
struct SetupCameraResult {
    handle: quiro_camera::CaptureHandle,
    camera_info: quiro_camera::CameraInfo,
    video_info: VideoInfo,
}
...
fn select_preferred_camera_format(
    formats: &[quiro_camera::Format],
    settings: CameraDeviceSettings,
) -> Option<quiro_camera::Format> {
...
fn select_camera_format(
    camera: &quiro_camera::CameraInfo,
    settings: Option<CameraDeviceSettings>,
) -> Result<quiro_camera::Format, SetInputError> {
...
async fn setup_camera(
    ...
    let first_attempt = start_camera_capture_attempt(
        &camera,
        format.clone(),
        quiro_camera::CaptureMode::Native,
        recipient.clone(),
    ...
    match first_attempt {
        ...
        Err(err @ (SetInputError::Timeout(_) | SetInputError::StartCapturing(_))) => {
            ...
            start_camera_capture_attempt(
                &camera,
                format,
                quiro_camera::CaptureMode::Compatibility,
                recipient,
                native_recipient,
                ffmpeg_sender_count,
                native_sender_count,
...
async fn start_camera_capture_attempt(
    camera: &quiro_camera::CameraInfo,
    format: quiro_camera::Format,
    mode: quiro_camera::CaptureMode,
    recipient: Recipient<NewFrame>,
    native_recipient: Recipient<NewNativeFrame>,
    ffmpeg_sender_count: Arc<std::sync::atomic::AtomicUsize>,
    native_sender_count: Arc<std::sync::atomic::AtomicUsize>,
    ...
    let capture_handle = camera
        .start_capturing_with_mode(format.clone(), mode, move |frame| {
            ...
            let timestamp =
                Timestamp::MachAbsoluteTime(quiro_timestamp::MachAbsoluteTimestamp::new(
    ...
    let capture_handle = camera
        .start_capturing(format.clone(), move |frame| {
            ...
            let timestamp = Timestamp::PerformanceCounter(
                quiro_timestamp::PerformanceCounterTimestamp::new(frame.native().perf_counter),
            );
