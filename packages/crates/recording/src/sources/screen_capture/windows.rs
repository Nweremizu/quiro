...
use quiro_timestamp::{PerformanceCounterTimestamp, Timestamp};
use scap_ffmpeg::*;
            ...
            move |data, info, config| {
                ...
                thread_local! {
                    static MMCSS_HANDLE: std::cell::RefCell<
                        Option<quiro_mediafoundation_utils::MmcssAudioHandle>,
                    > = const { std::cell::RefCell::new(None) };
                }
                MMCSS_HANDLE.with(|cell| {
                    let mut h = cell.borrow_mut();
                    if h.is_none() {
                        *h = quiro_mediafoundation_utils::MmcssAudioHandle::register_audio();
                    }
                });
...
impl output_pipeline::AudioSource for SystemAudioSource {
    ...
    fn audio_info(&self) -> quiro_media_info::AudioInfo {
        self.audio_info
    }
