...
use quiro_media_info::{AudioInfo, ffmpeg_sample_format_for};
use std::{
    sync::{
        Arc,
...
impl MicResampler {
    ...
    fn resample(
        ...
        source_channels: u16,
        source_format: SampleFormat,
        timestamp: quiro_timestamp::Timestamp,
    ) -> Option<AudioFrame> {
        let ffmpeg_fmt = ffmpeg_sample_format_for(source_format)?;
        let source_info = AudioInfo::new_raw(ffmpeg_fmt, source_rate, source_channels);
        ...
        async move {
            ...
            tokio::spawn({
                ...
                async move {
                    ...
                    let mut last_timestamp: Option<quiro_timestamp::Timestamp> = None;
...
mod tests {
    use super::*;
    use quiro_media_info::{Sample, Type};
    ...
    fn target_info_uses_output_rate_and_mono() {
        let device_info = AudioInfo {
            ...
            sample_rate: 32000,
            channels: 2,
            time_base: quiro_media_info::FFRational(1, 1_000_000),
            buffer_size: 1024,
