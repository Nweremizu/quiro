...
pub const AUDIO_OUTPUT_FRAMES: u32 = quiro_timestamp::AUDIO_OUTPUT_FRAMES as u32;
pub const DEFAULT_SAMPLE_RATE: u32 = quiro_timestamp::DEFAULT_SAMPLE_RATE;
...
impl AudioMixerBuilder {
    ...
    pub fn run(
        ...
        loop {
            ...
            #[cfg(target_os = "macos")]
            let now = Timestamp::MachAbsoluteTime(quiro_timestamp::MachAbsoluteTimestamp::now());
            #[cfg(windows)]
            let now =
                Timestamp::PerformanceCounter(quiro_timestamp::PerformanceCounterTimestamp::now());
...
mod test {
    ...
    const SOURCE_INFO: AudioInfo = AudioInfo::new_raw(
        quiro_media_info::Sample::U8(quiro_media_info::Type::Packed),
        SAMPLE_RATE,
        1,
    );
