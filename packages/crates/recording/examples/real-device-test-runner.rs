...
struct AvailableDevices {
    ...
    cameras: Vec<quiro_camera::CameraInfo>,
}
...
impl AvailableDevices {
    fn discover() -> anyhow::Result<Self> {
        ...
        let cameras: Vec<_> = quiro_camera::list_cameras().collect();
...
async fn execute_recording(
    ...
    #[cfg(target_os = "macos")]
    let shareable_content = quiro_recording::SendableShareableContent::from(
