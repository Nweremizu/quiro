...
async fn run_memory_test(
    ...
    if include_camera {
        if let Some(camera_info) = quiro_camera::list_cameras().next() {
...
async fn run_camera_only_test(duration_secs: u64) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(camera_info) = quiro_camera::list_cameras().next() {
        ...
        let (frame_tx, frame_rx) = flume::bounded::<quiro_recording::NativeCameraFrame>(128);
...
async fn run_cycles_test(
    ...
    for cycle in 1..=cycles {
        ...
        if include_camera {
            match quiro_camera::list_cameras().next() {
                Some(camera_info) => {
                    let feed = CameraFeed::spawn(CameraFeed::default());
