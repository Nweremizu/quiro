...
use quiro_audio::{AudioData, SyncAnalyzer};
use quiro_project::{RecordingMeta, RecordingMetaInner, StudioRecordingMeta};
use quiro_rendering::decoder::spawn_decoder;
use std::{
    fs,
    path::{Path, PathBuf},
...
fn report_to_markdown(report: &RecordingTestReport) -> String {
    ...
    for (i, result) in report.decoder_results.iter().enumerate() {
        ...
        if let Some(reason) = &result.fallback_reason {
            md.push_str(&format!("| ↳ Fallback | ⚠️ | {reason} |\n"));
        }
    }
