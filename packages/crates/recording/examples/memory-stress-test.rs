...
async fn test_camera_cycles(config: &CycleTestConfig) -> CycleTestResult {
    ...
    let Some(camera_info) = quiro_camera::list_cameras().next() else {
    ...
    for cycle in 0..config.cycles {
        ...
        let (frame_tx, frame_rx) = flume::bounded::<quiro_recording::FFmpegVideoFrame>(4);
...
async fn test_recording_cycles(
    ...
    for cycle in 0..config.cycles {
        ...
        let mut builder = quiro_recording::studio_recording::Actor::builder(
            dir.path().into(),
        ...
        if include_camera && let Some(camera_info) = quiro_camera::list_cameras().next() {
            let feed = CameraFeed::spawn(CameraFeed::default());
            feed.ask(camera::SetInput {
                settings: None,
        ...
        let handle = builder
            .build(
                #[cfg(target_os = "macos")]
                Some(quiro_recording::SendableShareableContent::from(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    unsafe { std::env::set_var("RUST_LOG", "info,quiro_recording=debug") };
