...
async fn run_camera_encoding_benchmark(
...
) -> quiro_recording::benchmark::MetricsSnapshot {
    use quiro_frame_converter::{
    ...
    let video_info = quiro_media_info::VideoInfo {
        width,
        height,
    ...
    let mut encoder = quiro_enc_ffmpeg::h264::H264Encoder::builder(video_info)
...
async fn main() {
    ...
    let cameras: Vec<_> = quiro_camera::list_cameras().map(CameraSelection).collect();
