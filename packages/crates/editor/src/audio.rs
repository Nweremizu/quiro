...
struct TimelineCursor<'a> {
    ...
    segment_time: f64,
    segment_index: usize,
    segment: &'a quiro_project::TimelineSegment,
}
...
fn render_audio_data_chunk(
    ...
    quiro_audio::render_audio(&track_datas, cursor.samples, samples, out_offset, out)
}

const SPEED_AUDIO_INPUT_BLOCK_SAMPLES: usize = 4_096;
const SPEED_AUDIO_PREROLL_SAMPLES: usize = 2_400;

fn project_stereo_mode_key(mode: &quiro_project::StereoMode) -> u8 {
    match mode {
        quiro_project::StereoMode::Stereo => 0,
        quiro_project::StereoMode::MonoL => 1,
        quiro_project::StereoMode::MonoR => 2,
    }
}
...
/// stereo; mono is centre-panned at -3dB to match `quiro_audio::render_audio`.
fn mix_music(
    music: &MusicTracks,
    timeline: &TimelineConfiguration,
    frame_start: usize,
...
fn spawn_progressive_render(
    ...
    let spawn_result = std::thread::Builder::new()
        .name("quiro-audio-prerender".into())
        .spawn(move || {
...
mod tests {
    ...
    fn music_track_segment(
        ...
        fade_in: f64,
        fade_out: f64,
    ) -> quiro_project::AudioTrackSegment {
        quiro_project::AudioTrackSegment {
            start,
            end,
            track: 0,
    ...
    fn music_project(
        audio_segments: Vec<quiro_project::AudioTrackSegment>,
