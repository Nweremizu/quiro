...
impl EditorInstance {
    ...
    pub async fn new_with_audio_output(
        ...
        let recording_meta = quiro_project::RecordingMeta::load_for_project(&project_path)
        ...
        if project.clips.is_empty() {
            ...
            match meta.as_ref() {
                StudioRecordingMeta::MultipleSegments { inner } => {
                    project.clips = inner
                        ...
                        .map(|(i, segment)| {
                            ...
                            quiro_project::ClipConfiguration {
                                index: i as u32,
                                offsets: segment
                ...
                StudioRecordingMeta::SingleSegment { .. } => {
                    project.clips = vec![quiro_project::ClipConfiguration {
                        index: 0,
                        offsets: quiro_project::ClipOffsets::default(),
...
impl LegacyAudioTimingRepair {
    ...
    fn offset(
        ...
        structured_summary: Option<&quiro_project::AudioGapSummary>,
    ) -> f32 {
    ...
    fn summary(
        ...
        segment_index: usize,
        track: LegacyAudioLogTrack,
    ) -> Option<quiro_project::AudioGapSummary> {
...
fn legacy_audio_gap_summary_from_log(
    ...
    let mut summary = quiro_project::AudioGapSummary {
...
fn audio_timing_repair_offset(summary: Option<&quiro_project::AudioGapSummary>) -> f32 {
...
pub async fn create_segments(
    ...
    match &meta {
        quiro_project::StudioRecordingMeta::SingleSegment { segment: s } => {
            let audio = s
                .audio
                .as_ref()
                .map(|audio_meta| {
        ...
        quiro_project::StudioRecordingMeta::MultipleSegments { inner, .. } => {
...
fn load_calibration_store(project_path: &std::path::Path) -> quiro_audio::CalibrationStore {
    ...
    quiro_audio::CalibrationStore::load(&calibration_dir)
}

fn get_calibration_offset(
    camera_id: Option<&str>,
    mic_id: Option<&str>,
    store: &quiro_audio::CalibrationStore,
) -> Option<f32> {
...
mod tests {
    use super::*;
    use quiro_project::AudioGapSummary;
