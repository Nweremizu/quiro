...
use quiro_audio::{AudioData, FromSampleBytes};
#[cfg(not(target_os = "windows"))]
use quiro_audio::{LatencyCorrectionConfig, LatencyCorrector, default_output_latency_hint};
...
impl AudioOutput {
    ...
    pub fn new_headless(tap: HeadlessAudioTap) -> Self {
        ...
        if let Err(e) = std::thread::Builder::new()
            .name("quiro-audio-headless".into())
...
fn control_thread_headless(control_rx: std_mpsc::Receiver<ControlMsg>, mut tap: HeadlessAudioTap) {
    ...
    let pump = {
        ...
        std::thread::Builder::new()
            .name("quiro-audio-headless-pump".into())
            .spawn(move || {
