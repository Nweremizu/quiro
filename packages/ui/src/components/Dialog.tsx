...
export function DialogHeader({
  ...
  children,
  ...props
}: HTMLAttributes<HTMLDivElement> & { icon?: JSX.Element; description?: string }) {
  return (
    <div
      className={cn(
...
export function DialogTitle({ className, ...props }: DialogPrimitive.Title.Props) {
  return (
    <DialogPrimitive.Title className={cn("text-lg font-medium text-gray-12", className)} {...props} />
  );
}

export function DialogDescription({ className, ...props }: DialogPrimitive.Description.Props) {
  return (
    <DialogPrimitive.Description className={cn("p-5 text-sm text-gray-11", className)} {...props} />
  );
}
