...
const IS_SMOKE_EXPORT = process.env.QUIRO_SMOKE_EXPORT === "1";
...
function getDefaultTrayIcon() {
  if (!defaultTrayIcon) {
    defaultTrayIcon = getTrayIcon("app-icons/quiro-32.png");
  }
...
function syncDockIcon() {
  ...
  const dockIcon = getAppImage("app-icons/quiro-512.png");
  if (!dockIcon.isEmpty()) {
...
//     case "error":
//       return `Quiro ${payload.version} needs attention`;
//   }
// }
...
//   switch (payload.phase) {
//     case "available":
//       return "Click to install the update and restart Quiro.";
//     case "downloading":
//       return "Quiro is downloading the update and will restart when it is ready.";
//     case "ready":
...
// function sendUpdateToastToWindows(
...
//       icon: getAppImage("app-icons/quiro-128.png"),
//       silent: false,
//     });
...
//   (_event, installAfterDownload?: boolean) => {
...
function updateTrayMenu(recording: boolean = false) {
  ...
  const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "Quiro";
...
app.whenReady().then(async () => {
  ...
  if (IS_SMOKE_EXPORT || process.env.QUIRO_DEV_OPEN_RECORDING_INPUT) {
    await logSmokeExportGpuDiagnostics();
    if (IS_SMOKE_EXPORT) {
      const smokeSource =
        process.env.QUIRO_SMOKE_EXPORT_PROJECT ??
        process.env.QUIRO_SMOKE_EXPORT_INPUT ??
        "<missing input>";
      console.log(
        `[smoke-export] Starting editor smoke export for ${smokeSource}`,
      );
    } else {
      console.log(
        `[dev-open-recording] Starting editor for ${process.env.QUIRO_DEV_OPEN_RECORDING_INPUT}`,
      );
    }
    createEditorWindowWrapper();
    return;
