...
import { RECORDINGS_DIR } from "./application-path";
...
export async function moveFileWithOverwrite(
  sourcePath: string,
...
async function loadRecordingsDirectorySetting() {
  ...
  try {
    ...
    const parsed = parseJsonWithByteOrderMark<{ recordingsDir?: unknown }>(
      content,
    );
    if (
      typeof parsed.recordingsDir === "string" &&
      parsed.recordingsDir.trim()
    ) {
...
export function getMacPrivacySettingsUrl(
  pane: "screen" | "accessibility" | "microphone",
): string {
  if (pane === "screen")
