...
import {
  RECORDINGS_DIR,
  USER_DATA_PATH,
} from "@electron/utils/application-path";
import { isSupportedLocalMediaPath } from "@/types";
import {
...
export function isAllowedLocalReadPath(candidatePath: string) {
  ...
  try {
    canonicalCandidatePath = normalizePath(
      realpathSync(normalizedCandidatePath),
    );
  } catch {
  ...
  return (
    allowedPrefixes.some((prefix) =>
      isPathInsideDirectory(canonicalCandidatePath, prefix),
...
async function collectApprovedLocalReadPaths(
  ...
  try {
    const realPath = await fs.realpath(approvedPaths[0] as string);
...
export async function resolveApprovedLocalMediaPath(
  candidatePath: string,
): Promise<string | null> {
  const normalizedCandidatePath = normalizePath(candidatePath);
...
> {
  ...
  const rawWebcamPath =
    typeof (project as { editor?: { webcam?: { sourcePath?: unknown } } })
      .editor?.webcam?.sourcePath === "string"
      ? ((project as { editor?: { webcam?: { sourcePath?: string } } }).editor
...
export function hasProjectFileExtension(filePath: string) {
  ...
  return [PROJECT_FILE_EXTENSION, ...LEGACY_PROJECT_FILE_EXTENSIONS].includes(
    extension,
  );
}
...
export async function saveProjectThumbnail(
  ...
  await fs.writeFile(thumbnailPath, Buffer.from(match[1] as string, "base64"));
...
export async function saveRecentProjectPaths(paths: string[]) {
  const normalizedPaths = Array.from(
    new Set(paths.map((value) => normalizePath(value))),
  ...
  try {
    ...
    return {
      ...
      isInProjectsDirectory:
        path.dirname(normalizedPath) === normalizePath(projectsDir),
    };
  } catch {
    return null;
  }
...
export async function listProjectLibraryEntries() {
  ...
  const candidatePaths = Array.from(
    new Set([...projectPaths, ...recentProjectPaths]),
  );
  const entries = (
    await Promise.all(
...
export async function loadProjectFromPath(projectPath: string) {
  ...
  const audioTracks = editorObj?.audioTracks as
    | { sourcePath?: unknown }[]
    | undefined;
