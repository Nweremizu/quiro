...
import { resolveLinuxWindowBounds } from "@electron/ipc/cursor/bounds";
...
export function getDisplayWorkAreaForSource(source: SelectedSource) {
  ...
  const matched =
    allDisplays.find((d: { id: number }) => d.id === displayId) ??
    primaryDisplay;
...
export async function buildFfmpegCaptureArgs(
  ...
  if (process.platform === "linux") {
    ...
    if (source?.id?.startsWith("window:")) {
      ...
      if (!bounds) {
        throw new Error(
          "Unable to resolve Linux window bounds for FFmpeg capture",
        );
      }
...
export function waitForFfmpegCaptureStart(
