...
import { COMPANION_AUDIO_LAYOUTS } from "@electron/utils/constants";
...
export async function getFileSizeIfPresent(
...
export async function getCompanionAudioFallbackInfo(videoPath: string) {
  ...
  if (await hasEmbeddedAudioStream(videoPath)) {
    ...
    paths = Array.from(
      new Set(
        companionCandidates.flatMap((candidate) => candidate.usablePaths),
      ),
    );
  }
...
export async function validateRecordedVideo(videoPath: string) {
  ...
  try {
    ...
    throw new Error(
      output || `Recorded output could not be decoded: ${videoPath}`,
    );
  }
