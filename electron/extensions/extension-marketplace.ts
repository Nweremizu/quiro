 ...
 * Quiro marketplace API. Also provides admin review endpoints.
 */
...
const MARKETPLACE_API_BASE = 'https://marketplace.quiro.dev/extensions/api/v1';
...
function getMarketplaceUrl(): string {
  ...
  if (process.env.QUIRO_MARKETPLACE_URL)
    return process.env.QUIRO_MARKETPLACE_URL;
  return MARKETPLACE_API_BASE;
}

function getAdminKey(): string | undefined {
  return process.env.QUIRO_ADMIN_KEY;
}
...
async function marketplaceFetch<T>(
  ...
  try {
    const headers: Record<string, string> = {
      ...
      'X-Quiro-Version': app.getVersion(),
      'X-Quiro-Platform': process.platform,
    };
    ...
    if (options.admin) {
      ...
      if (!key)
        throw new Error(
          'Admin key not configured (set QUIRO_ADMIN_KEY env var)'
        );
 ...
 */
export async function getMarketplaceExtension(
  id: string
...
export async function downloadAndInstallExtension(
  ...
  try {
    ...
    if (!allowedOrigins.some((o) => url.origin === o)) {
      return {
        success: false,
        error: `Untrusted download origin: ${url.origin}`,
      };
    }
  } catch {
  ...
  const tempDir = path.join(
    app.getPath('temp'),
    `quiro-ext-${extensionId}-${Date.now()}`
  );
  ...
  try {
    ...
    if (
      dirs.length === 1 &&
      !existsSync(path.join(extractDir, 'quiro-extension.json'))
    ) {
      if (dirs[0]) {
    ...
    if (!existsSync(path.join(manifestDir, 'quiro-extension.json'))) {
      throw new Error(
        'Downloaded extension does not contain a quiro-extension.json manifest'
      );
    }
    ...
    fetch(
      `${getMarketplaceUrl()}/extensions/${encodeURIComponent(extensionId)}/download`,
      {
        method: 'POST',
        headers: { 'X-Quiro-Version': app.getVersion() },
      }
    ).catch(() => undefined);
