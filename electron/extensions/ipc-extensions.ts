...
import type { ExtensionInfo, MarketplaceReviewStatus } from '@/types';
...
export function registerExtensionIpcHandlers(): void {
  ...
  ipcMain.handle('extensions:install-from-folder', async (event) => {
    ...
    const result = await dialog.showOpenDialog(window!, {
      ...
      message: 'Select a folder containing a quiro-extension.json manifest',
    });
    ...
    const info = await installExtensionFromPath(result.filePaths[0] as string);
    if (!info) {
      return {
        success: false,
        reason: 'Invalid extension: missing or invalid quiro-extension.json',
      };
    }
