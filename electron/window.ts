...
const HUD_OVERLAY_SETTINGS_FILE = path.join(
  USER_DATA_PATH,
  "hud-overlay-settings.json",
);
...
function getEditorWindowQuery(): Record<string, string> {
  ...
  if (process.env.QUIRO_DEV_OPEN_RECORDING_INPUT) {
    query.devOpenInput = process.env.QUIRO_DEV_OPEN_RECORDING_INPUT;
  }
  ...
  if (process.env.QUIRO_SMOKE_EXPORT === "1") {
    query.smokeExport = "1";
    if (process.env.QUIRO_SMOKE_EXPORT_INPUT) {
      query.smokeInput = process.env.QUIRO_SMOKE_EXPORT_INPUT;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_OUTPUT) {
      query.smokeOutput = process.env.QUIRO_SMOKE_EXPORT_OUTPUT;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_USE_NATIVE === "1") {
      query.smokeUseNativeExport = "1";
    }
    if (process.env.QUIRO_SMOKE_EXPORT_ENCODING_MODE) {
      query.smokeEncodingMode = process.env.QUIRO_SMOKE_EXPORT_ENCODING_MODE;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_SHADOW_INTENSITY) {
      query.smokeShadowIntensity =
        process.env.QUIRO_SMOKE_EXPORT_SHADOW_INTENSITY;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_WEBCAM_INPUT) {
      query.smokeWebcamInput = process.env.QUIRO_SMOKE_EXPORT_WEBCAM_INPUT;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_WEBCAM_SHADOW) {
      query.smokeWebcamShadow = process.env.QUIRO_SMOKE_EXPORT_WEBCAM_SHADOW;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_WEBCAM_SIZE) {
      query.smokeWebcamSize = process.env.QUIRO_SMOKE_EXPORT_WEBCAM_SIZE;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_PIPELINE) {
      query.smokePipelineModel = process.env.QUIRO_SMOKE_EXPORT_PIPELINE;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_BACKEND) {
      query.smokeBackendPreference = process.env.QUIRO_SMOKE_EXPORT_BACKEND;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_RENDER_BACKEND) {
      query.smokeRenderBackend = process.env.QUIRO_SMOKE_EXPORT_RENDER_BACKEND;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
      query.smokeMaxEncodeQueue =
        process.env.QUIRO_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
      query.smokeMaxDecodeQueue =
        process.env.QUIRO_SMOKE_EXPORT_MAX_DECODE_QUEUE;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
      query.smokeMaxPendingFrames =
        process.env.QUIRO_SMOKE_EXPORT_MAX_PENDING_FRAMES;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_PROJECT) {
      query.smokeProject = process.env.QUIRO_SMOKE_EXPORT_PROJECT;
    }
    if (process.env.QUIRO_SMOKE_EXPORT_QUALITY) {
      query.smokeQuality = process.env.QUIRO_SMOKE_EXPORT_QUALITY;
    }
...
function getUpdateToastBounds() {
  ...
  if (hudWindow) {
    ...
    const x = Math.round(
      hudBounds.x + (hudBounds.width - UPDATE_TOAST_WIDTH) / 2,
    );
    const y = Math.max(
...
ipcMain.on(
  "hud-overlay-drag",
  (_event, phase: string, screenX: number, screenY: number) => {
    ...
    if (phase === "start") {
      ...
      const fixedWidth =
        hudDragFixedSize?.width ?? hudOverlayWindow.getBounds().width;
      const fixedHeight =
        hudDragFixedSize?.height ?? hudOverlayWindow.getBounds().height;
...
ipcMain.handle(
  "set-hud-overlay-capture-protection",
...
export function getHudOverlayWindow(): BrowserWindow | null {
  return hudOverlayWindow && !hudOverlayWindow.isDestroyed()
    ? hudOverlayWindow
    : null;
}
...
export function getUpdateToastWindow(): BrowserWindow | null {
  return updateToastWindow && !updateToastWindow.isDestroyed()
    ? updateToastWindow
    : null;
}
