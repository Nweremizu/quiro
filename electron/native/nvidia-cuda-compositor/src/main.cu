...
namespace
{
    ...
    struct Options
    {
        ...
        std::string outputPath = "quiro-nvidia-cuda-compositor.h264";
    ...
    Options parseOptions(int argc, char **argv)
    {
        ...
        for (int index = 1; index < argc; ++index)
        {
            ...
            {
                std::cout << "Usage: quiro-nvidia-cuda-compositor --input input.annexb.h264 "
        ...
        void *userData)
    {
        ...
        while (*state->encodedFrames < expectedOutputFrames && *state->encodedFrames < maxOutputFrames)
        {
            ...
            (*state->sink)->encodeFrame(reinterpret_cast<const unsigned char *>(dpSrcFrame), static_cast<int>(nSrcPitch), width, height, surfaceHeight, *state->encodedFrames);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        const int reportedSourceFrames =
            useDecoderFramePolicy ? selectionState.sourceFrames : (options.callbackEncode ? decoder->GetDisplayFrameCount() : sourceFrames);
