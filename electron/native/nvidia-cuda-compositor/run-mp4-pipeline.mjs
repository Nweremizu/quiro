...
const unpackAsarPath = (candidate) =>
...
const ffmpegCommand = resolveToolCommand(
  ["QUIRO_FFMPEG_EXE"],
  "ffmpeg-static",
  "ffmpeg",
);
const ffprobeCommand = resolveToolCommand(
  ["QUIRO_FFPROBE_EXE"],
  "ffprobe-static",
  "ffprobe",
);
...
const cursorTypeIndexes = new Map(
  cursorTypes.map((type, index) => [type, index]),
);
...
function shouldRaiseChildPriority() {
  return process.env.QUIRO_NVIDIA_CUDA_EXPORT_HIGH_PRIORITY !== "0";
}
...
function raiseChildPriority(child, label) {
  ...
  try {
    ...
    console.warn(
      `[nvidia-cuda-export] Failed to raise ${label} priority: ${error}`,
    );
    return false;
  }
}
...
function runWithProgress(command, args, progressOptions, options = {}) {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    const maybeEmitProgress = (text) => {
      ...
      if (
        percentage >= lastEmittedPercentage + 0.25 ||
...
function parseFfmpegStatsFrameCount(stderr) {
  ...
  return Number.isFinite(frameCount) && frameCount > 0
...
function writeCursorSamples(cursorPayload, outputPath) {
  const samples = Array.isArray(cursorPayload.samples)
    ? cursorPayload.samples
    : [];
...
async function runWithGpuMonitor(command, args, sampleIntervalMs) {
  ...
  collectSample();
  const interval = shouldSampleGpu
    ? setInterval(collectSample, sampleIntervalMs)
    : null;
...
function getVideoInfo(inputPath) {
  ...
  if (stream.codec_name !== "h264") {
    fail(
      `The NVIDIA CUDA compositor currently expects H.264 input, got ${stream.codec_name}`,
    );
  }
...
function readTimelineSegments(timelineMapPath) {
  ...
  const segments = readFileSync(resolvedPath, "utf8")
    ...
    .filter(Boolean)
    .map((line) => {
      const [sourceStartMs, sourceEndMs, outputStartMs, outputEndMs, speed] =
...
async function writeFramePtsSidecarAsync(inputPath, durationSec, outputPath) {
  ...
  writeFileSync(
    outputPath,
    timestamps.map((value) => value.toFixed(9)).join("\n"),
  );
...
function createBackgroundFilter(videoInfo, shadowOptions, blurPx = 0) {
  const safeBlurPx = Math.max(
    0,
    Math.min(96, Math.round(Number.isFinite(blurPx) ? blurPx : 0)),
  );
  ...
  const shadowAlpha = Math.round(
    255 * Math.min(0.5, shadowOptions.intensityPct / 200),
  );
...
const outputPath = resolve(
  getArg("--output", join(scriptDir, "quiro-nvdec-nvenc-mp4-output.mp4")),
);
...
const gpuSampleIntervalMs = Math.round(
  getNumberArg("--gpu-sample-interval-ms", 1000),
);
...
const shadowOffsetY = Math.round(
  getNonNegativeNumberArg("--shadow-offset-y", 0),
);
const shadowIntensityPct = Math.round(
  getNonNegativeNumberArg("--shadow-intensity-pct", 0),
);
...
function resolveNativeProbePath() {
  const configuredPath = process.env.QUIRO_NVIDIA_CUDA_EXPORT_EXE;
  const platformArch = process.arch === "arm64" ? "win32-arm64" : "win32-x64";
  const candidates = [
    configuredPath,
    join(scriptDir, "build", "Release", "quiro-nvidia-cuda-compositor.exe"),
    join(
      repoRoot,
      "electron",
      "native",
      "bin",
      platformArch,
      "quiro-nvidia-cuda-compositor.exe",
    ),
    // Backward-compatible legacy helper path while old work dirs are being retired.
    join(scriptDir, "build", "Release", "quiro-nvdec-nvenc-probe.exe"),
  ].filter(Boolean);
  ...
  fail(
    `Native NVIDIA CUDA compositor is not built. Checked: ${candidates.join(", ")}`,
  );
}
...
const backgroundSuffix = backgroundSuffixParts.length
  ? `.${backgroundSuffixParts.join(".")}`
  : "";
...
if (
  timelineSegments.length &&
  Math.abs(durationSec - timelineOutputDurationSec) > 0.05
) {
  fail(
...
let sourceWindowFrames = Math.max(
  ...
  Math.min(
    ...
    Math.ceil(
      (videoInfo.sourceFrames * sourceDurationSec) / videoInfo.durationSec,
    ),
  ),
);
let webcamSourceWindowFrames = webcamInfo
  ? Math.max(
      ...
      Math.min(
        ...
        Math.ceil(
          (webcamInfo.sourceFrames * sourceDurationSec) /
            webcamInfo.durationSec,
        ),
      ),
    )
  : 0;
...
const sourcePtsPromise = writeFramePtsSidecarAsync(
  inputPath,
  sourceDurationSec,
  sourcePtsPath,
);
...
const [backgroundConvert, webcamConvert, webcamDemux, demux, sourcePts] =
  await Promise.all([
...
if (
  timelineSegments.length &&
  (!sourcePts.path || sourcePts.frames < sourceWindowFrames)
) {
  fail(
    "Timeline-map CUDA export requires source frame PTS for the full source window",
  );
}
...
const outputVideo =
  outputStreams.find((stream) => stream.codec_type === "video") ?? null;
const outputAudio =
  outputStreams.find((stream) => stream.codec_type === "audio") ?? null;
