...
function calculateAxisScale(visibleRangeMs: number): {
  intervalMs: number;
  gridMs: number;
} {
  ...
  const candidate =
    SCALE_CANDIDATES.find((scaleCandidate) => {
      ...
      return (
        visibleSeconds / scaleCandidate.intervalSeconds <= TARGET_MARKER_COUNT
      );
...
function TimelineAxis({
  ...
  // Keep axis width up-to-date for edge anchoring logic
  useEffect(() => {
    if (!axisRef.current) return;
    const setWidth = () => setAxisWidth(axisRef.current?.clientWidth || 0);
    setWidth();
    const ro = new ResizeObserver(() => setWidth());
    ro.observe(axisRef.current);
  ...
  return (
    ...
    >
      ...
      {markers.markers.map((marker) => {
        ...
        // Smart anchoring: prefer centered labels, but anchor to left/right
        // when markers are within 40px of the axis edges to avoid clipping.
        let transformValue: string = "translateX(-50%)";
        if (axisWidth > 0) {
          if (offset < 40) transformValue = "translateX(0)";
          else if (axisWidth - offset < 40)
...
function Timeline({
  ...
  const audioRowIds = useMemo(
    () =>
      Array.from(
        new Set(
          audioItems.map((item) =>
            getAudioTrackRowId(getAudioTrackIndex(item.rowId)),
          ),
        ),
      ).sort(
  ...
  const annotationRowIds = useMemo(
    () =>
      Array.from(
        ...
        (left, right) =>
          getAnnotationTrackIndex(left) - getAnnotationTrackIndex(right),
      ),
    [annotationItems],
  );
  ...
  const ghostStartOffsetPx =
    ghostStartMs === null
      ? 0
      : valueToPixels(Math.max(0, ghostStartMs - range.start));
  const ghostEndOffsetPx =
    ghostEndMs === null
      ? 0
      : valueToPixels(Math.max(0, ghostEndMs - range.start));
  const ghostWidthPx = Math.max(18, ghostEndOffsetPx - ghostStartOffsetPx);
  const timelineGhostOffsetPx =
    timelineHoverMs === null
      ? 0
      : valueToPixels(Math.max(0, timelineHoverMs - range.start));
      ...
      >
        ...
        {audioRowIds.map((rowId, index) => {
          const rowItems = audioItems.filter(
            (item) =>
              getAudioTrackRowId(getAudioTrackIndex(item.rowId)) === rowId,
          );
...
const TimelineEditor = forwardRef<TimelineEditorHandle, TimelineEditorProps>(
  function TimelineEditor(
    ...
    const deleteSelectedAnnotation = useCallback(() => {
      if (!selectedAnnotationId || !onAnnotationDelete || !onSelectAnnotation)
        return;
    ...
    useEffect(() => {
      ...
      zoomRegionsRef.current.forEach((region) => {
        ...
        if (
          normalizedStart !== region.startMs ||
          normalizedEnd !== region.endMs
        ) {
          onZoomSpanChange(region.id, {
    ...
    const hasOverlap = useCallback(
      (newSpan: Span, excludeId?: string, rowId?: string): boolean => {
        ...
        const isAnnotationItem = annotationRegions.some(
          (r) => r.id === excludeId,
        );
    ...
    const handleAddAudio = useCallback(
      async (preferredTrackIndex?: number) => {
        ...
        const normalizedPreferredTrackIndex = Number.isFinite(
          preferredTrackIndex,
        )
        ...
        const getGapForTrack = (trackIndex: number) => {
          ...
          const nextRegion = trackRegions.find(
            (region) => region.startMs > startPos,
          );
          return nextRegion
            ? nextRegion.startMs - startPos
            : totalMs - startPos;
    ...
    const timelineItems = useMemo<TimelineRenderItem[]>(() => {
      ...
      const annotations: TimelineRenderItem[] = annotationRegions.map(
        (region) => {
          let label: string;
    ...
    const getResolvedDropRowId = useCallback(
      (id: string, proposedRowId: string) => {
        const currentRowId = timelineItems.find(
          (item) => item.id === id,
        )?.rowId;
        if (!currentRowId) {
        ...
        if (zoomRegions.some((r) => r.id === id)) {
          ...
          const nextTrackIndex =
            rowId && isAnnotationTrackRowId(rowId)
              ...
              : (annotationRegions.find((region) => region.id === id)
                  ?.trackIndex ?? 0);
          ...
          const nextTrackIndex =
            rowId && isAudioTrackRowId(rowId)
              ? getAudioTrackIndex(rowId)
              : (audioRegions.find((region) => region.id === id)?.trackIndex ??
                0);
      ...
      (deltaMs: number) => {
        ...
        setRange((previous) => {
          ...
          const nextStart = Math.max(
            0,
            Math.min(previous.start + deltaMs, maxStart),
          );
          ...
          <div className="flex items-center gap-2 px-4 py-2 border-b border-foreground/10 bg-editor-panel">
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                title="Split Clip (C)"
              >
                <HugeiconsIcon icon={Scissors} className="w-4 h-4" />
              </Button>
            </div>
            <div className="flex items-center gap-2">
              ...
              >
                <HugeiconsIcon icon={Crop} className="w-3.5 h-3.5" />
                <span className="font-medium">
