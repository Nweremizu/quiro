...
export function normalizeExportBackendPreference(
  value: unknown,
): ExportBackendPreference {
...
export function normalizeExportPipelineModel(
  value: unknown,
): ExportPipelineModel {
...
export function normalizeExportMp4FrameRate(
  value: unknown,
): ExportMp4FrameRate {
...
export function validateProjectData(
  ...
  if (project.projectId !== undefined && typeof project.projectId !== "string")
    return false;
...
export function normalizeProjectEditor(
  ...
  const rawZoomMotionBlurTuning =
    ...
    typeof (editor as Partial<ProjectEditorState>).zoomMotionBlurTuning ===
      "object"
  ...
  const normalizedZoomTemporalMotionBlur = isFiniteNumber(
    ...
    ? clamp(
        (editor as Partial<ProjectEditorState>)
          .zoomTemporalMotionBlur as number,
        0,
        2,
      )
  ...
  const normalizedZoomMotionBlurShutterFraction = isFiniteNumber(
    ...
    ? clamp(
        (editor as Partial<ProjectEditorState>)
          .zoomMotionBlurShutterFraction as number,
        TEMPORAL_MOTION_BLUR_MIN_SHUTTER_FRACTION,
        TEMPORAL_MOTION_BLUR_MAX_SHUTTER_FRACTION,
      )
    : TEMPORAL_MOTION_BLUR_DEFAULT_SHUTTER_FRACTION;
  ...
  const normalizedAutoFullTrackClipId =
    typeof editor.autoFullTrackClipId === "string"
      ? editor.autoFullTrackClipId
      : null;
  const normalizedAutoFullTrackClipEndMs = isFiniteNumber(
    editor.autoFullTrackClipEndMs,
  )
        ...
        .map((region, index) => {
          ...
          return {
            ...
            blurIntensity: isFiniteNumber(region.blurIntensity)
              ...
              : 20,
            blurColor:
              typeof region.blurColor === "string"
                ? region.blurColor
                : undefined,
    ...
    ? ((editor as Partial<ProjectEditorState>).audioRegions as AudioRegion[])
        ...
        .map((region) => {
          ...
          return {
            ...
            endMs,
            audioPath:
              typeof region.audioPath === "string" ? region.audioPath : "",
            volume: isFiniteNumber(region.volume)
              ? clamp(region.volume, 0, 1)
              : 1,
            trackIndex: isFiniteNumber(region.trackIndex)
              ? Math.max(0, Math.floor(region.trackIndex))
              : 0,
  ...
  const normalizedAutoCaptions: CaptionCue[] = Array.isArray(
    ...
    ? ((editor as Partial<ProjectEditorState>).autoCaptions as CaptionCue[])
        .filter((cue): cue is CaptionCue =>
          Boolean(cue && typeof cue.id === "string"),
        )
        .map((cue) => {
          const rawStart = isFiniteNumber(cue.startMs)
            ? Math.round(cue.startMs)
            : 0;
  ...
  const webcamSourcePath =
    typeof webcam.sourcePath === "string" ? webcam.sourcePath : null;
  ...
  const normalizedCursorStyle =
    typeof editor.cursorStyle === "string" &&
    editor.cursorStyle.trim().length > 0
  ...
  const normalizedMotionValues = {
    ...
    cursorClickBounceDuration: isFiniteNumber(
      ...
      ? clamp(
          (editor as Partial<ProjectEditorState>)
            .cursorClickBounceDuration as number,
          60,
          500,
        )
  ...
  return {
    ...
    cameraSpringStiffnessMultiplier: isFiniteNumber(
      ...
      : 1,
    cameraSpringDampingMultiplier: isFiniteNumber(
      editor.cameraSpringDampingMultiplier,
    )
      ? clamp(editor.cameraSpringDampingMultiplier, 0.25, 3)
      : 1.13,
    cameraSpringMassMultiplier: isFiniteNumber(
      editor.cameraSpringMassMultiplier,
    )
    ...
    padding: (() => {
      ...
      if (p && typeof p === "object") {
        ...
        return {
          ...
          left: isFiniteNumber(p.left)
            ? clamp(p.left, 0, 100)
            : DEFAULT_PADDING.left,
          right: isFiniteNumber(p.right)
            ? clamp(p.right, 0, 100)
    ...
    webcam: {
      ...
      mirror:
        typeof webcam.mirror === "boolean"
          ? webcam.mirror
          : DEFAULT_WEBCAM_OVERLAY.mirror,
      ...
      size: isFiniteNumber(webcam.size)
    ...
    exportBackendPreference: normalizeExportBackendPreference(
      editor.exportBackendPreference,
    ),
    exportPipelineModel: normalizeExportPipelineModel(
      editor.exportPipelineModel,
    ),
    exportQuality:
