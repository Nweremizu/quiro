...
type StatefulCursorStyle = Extract<
  CursorStyle,
  "macos" | "tahoe" | "tahoe-inverted"
>;
type SingleCursorStyle = Extract<CursorStyle, "dot" | "figma">;
type CursorPackStyle = Exclude<
  CursorStyle,
  StatefulCursorStyle | SingleCursorStyle
>;
...
let loadedCursorSetAssets: Partial<
  Record<
    StatefulCursorStyle,
    Partial<Record<CursorAssetKey, LoadedCursorAsset>>
  >
> = {};
...
const CURSOR_PACK_POINTER_TYPES = new Set<CursorAssetKey>([
  "pointer",
  "open-hand",
  "closed-hand",
]);
...
function getCursorPackSources(): Record<string, CursorPackSource> {
  const sources: Record<string, CursorPackSource> = {
    ...BUILTIN_CURSOR_PACK_SOURCES,
  };

  for (const cursorStyle of extensionHost.getContributedCursorStyles()) {
    const hotspot =
      cursorStyle.cursorStyle.hotspot ?? DEFAULT_CURSOR_PACK_ANCHOR;
    sources[cursorStyle.id] = {
      defaultUrl: cursorStyle.resolvedDefaultUrl,
      pointerUrl:
        cursorStyle.resolvedClickUrl ?? cursorStyle.resolvedDefaultUrl,
...
function buildCursorPackSourcesSignature(
  sources: Record<string, CursorPackSource>,
): string {
...
function isStatefulCursorStyle(
  style: CursorStyle,
): style is StatefulCursorStyle {
...
function resolveCursorPackVariant(
  cursorType: CursorAssetKey,
): CursorPackVariant {
    ...
    return {
      ...
      anchorX:
        trimmed.hotspot && trimmed.width > 0
          ? trimmed.hotspot.x / trimmed.width
          : 0,
      anchorY:
        trimmed.hotspot && trimmed.height > 0
          ? trimmed.hotspot.y / trimmed.height
          : 0,
    };
  }
      ...
      const cursorPackEntries = await Promise.all(
        Object.entries(sources).map(async ([style, source]) => {
          try {
            ...
            return [
              style,
              { default: defaultAsset, pointer: pointerAsset },
            ] as const;
          } catch (error) {
            console.warn(
  ...
  if (!cursorAssetsPromise) {
    cursorAssetsPromise = (async () => {
      async function loadCursorSet(
        ...
        const entries = await Promise.all(
          SUPPORTED_CURSOR_KEYS.map(async (key) => {
            ...
            if (!sourceAsset?.url) {
              console.warn(
                `[CursorRenderer] No cursor image for: ${style}/${key}`,
              );
              return null;
            }
      ...
      const invertedEntries = await Promise.all(
        (
          Object.entries(tahoeAssets) as Array<
            [CursorAssetKey, LoadedCursorAsset]
          >
        ).map(
          async ([key, asset]) =>
            [key, await createInvertedCursorAsset(asset)] as const,
        ),
      );
      ...
      const customStyleEntries = await Promise.all(
        (["dot", "figma"] as const).map(
          async (style) =>
            [style, await createCursorStyleAsset(style)] as const,
        ),
      );

      loadedCursorStyleAssets = Object.fromEntries(
        customStyleEntries,
      ) as Partial<Record<SingleCursorStyle, LoadedCursorAsset>>;
...
export async function buildNativeCursorAtlas(
  ...
  const atlasHeight =
    NATIVE_CURSOR_ATLAS_DRAW_HEIGHT + NATIVE_CURSOR_ATLAS_PADDING * 2;
  const canvas = document.createElement("canvas");
...
export class SmoothedCursorState {
  ...
  constructor(
    config: Pick<
      CursorRenderConfig,
      "smoothingFactor" | "trailLength" | "springTuning"
    >,
  ) {
...
export class PixiCursorOverlay {
  ...
  update(
    ...
    if (isStatefulCursorStyle(this.config.style)) {
      ...
      for (const currentShadowSprite of Object.values(
        this.cursorShadowSprites,
      )) {
...
export function drawCursorOnCanvas(
  ...
  const drawHeight =
    h * bounceScale * getCursorStyleSizeMultiplier(config.style);
