import type { CursorTelemetryPoint, TrimRegion } from "@/types/editor";
...
export function buildLoopedCursorTelemetry(
  ...
  const timelineStartClampedMs = clamp(
  ...
  const boundedSamples = samples.filter(
    (sample) =>
      sample.timeMs >= timelineStartClampedMs && sample.timeMs <= timelineEndMs,
  );
  ...
  const freezeDurationMs = Math.min(
    LOOP_CURSOR_FREEZE_DURATION_MS,
    maxFreezeWindowMs,
  );
  ...
  const returnMotionDurationMs = Math.max(
    1,
    freezeDurationMs - clampedSettleDurationMs,
  );
  const sourceStartMs = firstSample.timeMs;
  const sourceEndMs = Math.max(
    sourceStartMs,
    findLastMovingSampleTime(boundedSamples),
  );
  ...
  for (const sample of boundedSamples) {
    const progress = clamp(
      (sample.timeMs - sourceStartMs) / sourceDurationMs,
      0,
      1,
    );
    const mappedTimeMs = Math.round(
      timelineStartClampedMs + motionDurationMs * progress,
    );
  ...
  const returnStartPoint = interpolateCursorPosition(
    loopedSamples,
    motionEndMs,
  ) ?? {
    cx: lastSample.cx,
  ...
  const returnStartCursorType = findLatestStableCursorType(
    loopedSamples,
    motionEndMs,
  );
  ...
  for (let step = 0; step <= LOOP_CURSOR_RETURN_STEPS; step += 1) {
    ...
    const timeMs = Math.round(
      returnMotionStartMs + returnMotionDurationMs * progress,
    );
    loopedSamples.push({
      timeMs,
      cx:
        returnStartPoint.cx +
        (firstSample.cx - returnStartPoint.cx) * easedProgress,
      cy:
        returnStartPoint.cy +
        (firstSample.cy - returnStartPoint.cy) * easedProgress,
  ...
  if (clampedSettleDurationMs > 0) {
    ...
    for (let step = 1; step <= settleSteps; step += 1) {
      ...
      const timeMs = Math.round(
        settleStartMs + clampedSettleDurationMs * progress,
      );
      loopedSamples.push({
        timeMs,
        cx: firstSample.cx,
...
export function getDisplayedTimelineEndMs(
  totalDurationMs: number,
...
export function getDisplayedTimelineWindowMs(
  totalDurationMs: number,
  trimRegions: TrimRegion[],
) {
