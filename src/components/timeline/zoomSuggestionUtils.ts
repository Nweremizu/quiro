...
const EXPLICIT_CLICK_TYPES = new Set<
  NonNullable<CursorTelemetryPoint["interactionType"]>
...
function isExplicitClickType(
  ...
  return (
    ...
    EXPLICIT_CLICK_TYPES.has(interactionType)
  );
}
...
export function normalizeCursorTelemetry(
  ...
  for (const candidate of interactions) {
    ...
    if (
      candidate.kind === "text-field-click" ||
      candidate.kind === "text-focus-like"
    ) {
      applyCursorTypeInRange(
        normalized,
  ...
  for (const sample of normalized) {
    ...
    const mouseUp = normalized.find(
      (candidate) =>
        candidate.timeMs > sample.timeMs &&
        candidate.interactionType === "mouseup",
    );
    if (!mouseUp) {
      continue;
    }
    ...
    const dragDistance = Math.hypot(
      mouseUp.cx - sample.cx,
      mouseUp.cy - sample.cy,
    );
    if (dragDuration >= 160 && dragDistance > 0.015) {
      const isTextDrag =
        Math.abs(mouseUp.cx - sample.cx) >
        Math.abs(mouseUp.cy - sample.cy) * 1.8;
      applyCursorTypeInRange(
        normalized,
        sample.timeMs,
        mouseUp.timeMs,
...
export function detectZoomDwellCandidates(
  ...
  const pushRunIfDwell = (startIndex: number, endIndexExclusive: number) => {
    ...
    const avgCx =
      runSamples.reduce((sum, sample) => sum + sample.cx, 0) /
      runSamples.length;
    const avgCy =
      runSamples.reduce((sum, sample) => sum + sample.cy, 0) /
      runSamples.length;
...
export function detectInteractionCandidates(
  ...
  const clickEvents = samples.filter((sample) =>
    isExplicitClickType(sample.interactionType),
  );
  ...
  const dwellCandidates = detectZoomDwellCandidates(
    samples,
  ...
  const sortedByTime = [...dwellCandidates].sort(
  ...
  for (let index = 1; index < sortedByTime.length; index += 1) {
    ...
    const spatialGap = Math.hypot(
      curr.focus.cx - prev.focus.cx,
      curr.focus.cy - prev.focus.cy,
    );
