...
function isBundledAssetPath(resource: string) {
  return BUNDLED_ASSET_PATH_PREFIXES.some((prefix) =>
...
function isRemoteMediaResource(resource: string) {
  return (
    REMOTE_MEDIA_URL_PATTERN.test(resource) && !isLocalMediaServerUrl(resource)
  );
}
...
function inferMimeType(filePath: string) {
  const normalized =
    filePath.split("?")[0]?.toLowerCase() ?? filePath.toLowerCase();
...
export async function resolveMediaResourceUrl(
  resource: string,
): Promise<string> {
  const localFilePath = getLocalFilePath(resource);
  if (!localFilePath) {
...
export async function createReadableMediaResourceFile(
  resource: string,
): Promise<File> {
  const localFilePath = getLocalFilePath(resource);
  const filename =
    (localFilePath ?? resource).split(/[\\/]/).pop()?.split("?")[0] || "media";
