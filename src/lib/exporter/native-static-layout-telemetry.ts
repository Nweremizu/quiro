import type { CropRegion, CursorTelemetryPoint } from "@/types/editor";
import { projectCursorPositionToViewport } from "@/components/editor/videoPlayback/cursorViewport";
...
function isCursorInteractionSample(
...
function sanitizeCursorTelemetry(
...
function getCursorBounceScale(
  ...
  return Math.max(
    0.72,
    1 - Math.sin(progress * Math.PI) * (0.08 * clickBounce),
  );
}
...
function createMonotonicCursorRenderSampler(
  ...
  return (timeMs: number): NativeStaticLayoutCursorTelemetrySample => {
    ...
    while (
      metadataIndex < samples.length &&
      samples[metadataIndex].timeMs <= timeMs
    ) {
...
export function buildNativeStaticLayoutCursorTelemetry(
  ...
  const targetFrames = Math.max(
    1,
    Math.floor(durationMs / frameDurationMs) + 1,
  );
