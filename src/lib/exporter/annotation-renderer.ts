...
function getAnnotationImageContent(
...
export async function preloadAnnotationAssets(
  ...
  return {
    imageCache: new Map(
      loadedSources.filter(
        (entry): entry is readonly [string, HTMLImageElement] => !!entry,
      ),
    ),
  };
}
...
async function renderImage(
  ...
  const img =
    assets?.imageCache.get(source) ?? (await loadAnnotationImage(source));
  if (!img) {
    return;
  }
